...
| 3 | Missing `encryption:cenc-ctr-clear-output` feature | `encryption/cenc_ctr_decrypt`, `encryption/cenc_ctr_decrypt_eq_cleartext`, `encryption/perf_cenc_ctr_decrypt_throughput` | The adapter already implements CENC decrypt via `resolveKeyId` and remuxes plaintext MP4, but does not declare the export feature required by these scenarios. This should be validated and declared if the output is actually clear. |
...
| --- | ---: | --- | --- |
| `mediabunny@1.48.0` | 19 | Declared `fastStart:in-memory`, CENC-CTR clear-output export, alpha transcode, audio resample/downmix/upmix/gain/fade, crop, and pad capability. Implemented gain/fade through `ConversionAudioOptions.process`, crop through `ConversionVideoOptions.crop`, and pad/letterbox through `fit: 'contain'`. | Static negotiation against the stored run: 17 rows now negotiate, 2 alpha rows now move to `NA_BROWSER` under Chromium's alpha/WebCodecs gate. |
...
1. Fix the reference engine first: Mediabunny audio DSP, HLS AES-128, CENC clear output declaration, `fastStart:in-memory`, and alpha transcode. This removes false N/A from the baseline engine and improves oracle coverage.
