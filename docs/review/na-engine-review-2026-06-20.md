...
| 15 | Missing video/filter features: `alpha`, `depth:10bit`, `flip`, `colorspace`, `crf`, `crop`, `pad`, `two-pass`, `tonemap` | Transcode/trim/decode feature rows | FFmpeg supports filtergraphs and encoding controls. Current worktree implements and verifies `flip`, `crop`, `pad`, `colorspace`, `crf`, `two-pass`, and VP8 alpha-preserving transcode, declares generic alpha decode support, and gives VP9 alpha output an explicit runtime N/A for the vendored wasm core trap; remaining alpha trim behavior and depth/tonemap still need focused implementation or runtime N/A reasons. |
...
| 4 | Missing `mux:vfr-timestamps` | VFR/B-frame mux scenarios | Resolved in the current worktree: prepared mux tracks retain source-container bytes and `mux()` stream-copies mapped source streams so PTS/DTS tables survive. See focused Chromium evidence below. |
...
| 2 | Missing `metadata:protected-tracks` | `probe/cenc_cbcs`, `probe/cenc_ctr` | Current worktree declares this read-side capability and adds a narrow CENC MP4 `moov`/`moof` metadata fallback so protected track type/codec/duration/fps fields compare against goldens without decrypting. |
...
| Rows | Current NA reason | Affected cases | Why this needs work |
| ---: | --- | --- | --- |
| 19 | Missing video-only transform features (`fps`, `rotate`, `flip`, `crop`, `pad`, `alpha:transcode`, `fragmented`) | Mostly `transcode/*` feature rows plus decode-side rotation gates | Current worktree adds a decode-side MP4 display-matrix route through the browser `<video>` presenter and verifies the two committed rotation decode/read rows. Rotate transcode rows are now explicit runtime N/A in Chromium because the source carries audio or requests non-zero rotation application that the MediaRecorder canvas path still does not implement. Remaining video transform rows still need focused validation before changing capabilities. |
| 48 | Missing `wav` input container | Audio-DSP/probe/demux rows | Current worktree adds a narrow WAV header/parser path for platform probe/demux and verifies the eight committed WAV read rows. Remaining WAV rows that require audio decode/transcode/DSP are not counted until their op path is implemented or reclassified. |
...
| `ffmpeg.wasm@0.12.15` | 52 | Added AIFF/CAF containers, `pcm-s24be`, audio DSP declarations and mapping (`resample`, `downmix`, `upmix`, `gain`, `fade`), `decode:golden-rgba`, `fastStart:in-memory`, `metadata:write` tag forwarding through the runner, HLS playlist sidecar materialization for `.m3u8` inputs including AES-128 key files, FFmpeg filter/control support for `flip`, `crop`, `pad`, `colorspace`, `crf`, and `two-pass`, generic alpha decode capability, VP8 alpha-preserving transcode, and `mux:vfr-timestamps` through a source-copy mux path that preserves source PTS/DTS tables. Added explicit suite-budget/runtime N/A guards for browser-wasm paths that should not be attempted as ordinary transcodes/trims, including VP9 alpha output in the vendored wasm libvpx-vp9 path. | Static negotiation against the stored run for rows whose original reason was one of the changed declarations. The four HLS rows (`probe/hls_vod`, `probe/hls_aes128`, `demux/hls_vod`, `demux/hls_aes128`) were verified in Chromium and all passed in `results/raw/chromium-2026-06-20T20-36-16-380Z.json`. The transform/control rows were verified in Chromium: `transcode/h264_pad_letterbox_4x3_to_16x9` and `transcode/h264_flip_vertical` passed in `results/raw/chromium-2026-06-20T23-36-23-482Z.json`; `transcode/h264_crop_center` passed in `results/raw/chromium-2026-06-20T23-46-18-400Z.json`; `transcode/h264_flip_horizontal` and `transcode/h264_crf_quality_mode` passed in `results/raw/chromium-2026-06-20T23-48-47-703Z.json`; `transcode/h264_colorspace_709_to_2020` passed in `results/raw/chromium-2026-06-21T00-49-24-632Z.json`; `transcode/h264_two_pass_bitrate` passed in `results/raw/chromium-2026-06-21T01-01-25-096Z.json` with a 300s per-scenario timeout for the genuine two-pass encode. Generic alpha moved `decode-seek/decode_vp9_alpha` to `NA_ASSET` in `results/raw/chromium-2026-06-21T00-01-34-207Z.json`; `transcode/vp9_alpha_to_vp8_keepalpha` passed in `results/raw/chromium-2026-06-21T01-11-42-281Z.json`; `transcode/vp9_alpha_to_vp9_keepalpha` now returns an explicit runtime `NA_ENGINE` in `results/raw/chromium-2026-06-21T01-13-11-577Z.json`; `trim/vp9_alpha_keyframe_aligned` returns an explicit runtime `NA_ENGINE` in `results/raw/chromium-2026-06-21T00-04-19-958Z.json`. `transcode/h264_to_hevc_mp4` is explicitly disabled as a browser-wasm suite-budget `NA_ENGINE` in `results/raw/chromium-2026-06-21T00-04-19-958Z.json`, and `transcode/h264_to_vp8_webm` is explicitly disabled as a browser-wasm suite-budget `NA_ENGINE` in `results/raw/chromium-2026-06-21T01-17-30-958Z.json`. The four `mux:vfr-timestamps` rows (`mux/prop_vfr_mux_duration_mp4_to_mp4`, `mux/prop_vfr_mux_duration_mp4_to_mkv`, `mux/edge_bframes_decode_mux_mp4`, `mux/edge_bframes_decode_mux_mkv`) passed in `results/raw/chromium-2026-06-21T00-41-47-366Z.json`; the MKV VFR row uses a 200 ms scenario tolerance for Matroska final-sample duration materialization. Opus/runtime-NA rows are intentionally not counted as fixed. |
| `remotion-media-parser@4.0.479` | 17 | Declared MOV/MKV input containers and restored collapsed parser container identity from fixture id/mime for golden comparison. Declared `metadata:protected-tracks` and added a CENC MP4 metadata fallback that recovers protected track type/codec/dimensions/channels plus fragmented MP4 duration/fps from `moov`/`moof` tables. | Static negotiation against the stored run for MOV/MKV read rows. The protected-track probe rows `probe/cenc_ctr` and `probe/cenc_cbcs` passed in focused Chromium run `results/raw/chromium-2026-06-21T00-15-17-225Z.json`. |
| `platform@chrome-149` | 14 | Declared WAV/PCM read capability and added a header-derived WAV probe/demux path, including WAVE_FORMAT_EXTENSIBLE PCM for 24-bit fixtures. Added MP4 display-matrix detection so rotated MP4 decode uses the browser `<video>` presenter path, then declared decode-side rotation support. | Focused Chromium run passed `probe/wav_s16`, `probe/wav_s24`, `probe/wav_f32`, `probe/empty-audio-wav`, `demux/wav_s16`, `demux/wav_s24`, `demux/wav_f32`, and `demux/empty_audio_zero_packets` in `results/raw/chromium-2026-06-21T00-21-46-717Z.json`. Rotation decode/read rows `decode-seek/decode_rotated_display_matrix` and `metadata/rotation_decode_read_h264_rotated90` passed in `results/raw/chromium-2026-06-21T00-29-13-918Z.json`. The four platform rotate-transcode rows now produce explicit runtime `NA_ENGINE` reasons in `results/raw/chromium-2026-06-21T00-29-39-887Z.json`. |
...
- Focused Chromium runtime run passed FFmpeg VP8 alpha-preserving transcode: `transcode/vp9_alpha_to_vp8_keepalpha`.
- Focused Chromium runtime run confirmed FFmpeg VP9 alpha output is an explicit runtime `NA_ENGINE`, not a declaration skip: `transcode/vp9_alpha_to_vp9_keepalpha`.
- Focused Chromium runtime runs confirmed the FFmpeg alpha and HEVC budget outcomes: `decode-seek/decode_vp9_alpha` is now `NA_ASSET` until frame-bake goldens exist, `trim/vp9_alpha_keyframe_aligned` is an explicit runtime `NA_ENGINE`, and `transcode/h264_to_hevc_mp4` is an explicit browser-wasm suite-budget `NA_ENGINE`.
- Focused Chromium runtime run passed all four newly live FFmpeg mux timestamp rows: `mux/prop_vfr_mux_duration_mp4_to_mp4`, `mux/prop_vfr_mux_duration_mp4_to_mkv`, `mux/edge_bframes_decode_mux_mp4`, and `mux/edge_bframes_decode_mux_mkv`.
- Focused Chromium runtime run passed the two Remotion Media Parser protected-track probe rows: `probe/cenc_ctr` and `probe/cenc_cbcs`.
