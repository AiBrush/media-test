...
There is no operator-selected file in the run filter. The UI supplies engine, feature, scenario, timing, reuse, and exhaustive flags; `runMatrix()` loads the scenario source catalog and chooses one canonical input [variant](../glossary.md) per scenario, or every candidate in exhaustive mode, consistently across engines.

Execution order is deterministic scenario-major order. Canonical media selection is stable across reloads, so a compatible cached cell keeps the same input identity and can be reused in subset-to-superset runs.
...
The matrix renderer constructs engine instance ids before execution so streamed versioned ids can map back to the columns. It normalizes each cell key by dropping the `@version` suffix; the app and runner use the same deterministic `buildExecutionOrder()` helper.
...
The page exports only rows from the current validation epoch. Current-schema rows retain their invalidation flag for analysis, while incompatible prior-epoch rows are excluded without migration and counted in both the bundle and the completion message. The page can clear the whole store or every exact/versioned `aibrush-media` engine id after a native confirmation. Cache controls are disabled during a run and when IndexedDB is absent.

IndexedDB is origin-scoped. `run.sh` prefers the remembered cache-origin port when available, and cross-origin cache transfer requires an explicit validated export/import operation.

### Download, launcher, and serving interface

Cache export emits `media-browser-test/browser-cache-export@3` with its current validation epoch, origin, stored/exported/excluded counts, current-schema entry wrappers, and a content hash. Removed fields and prior-epoch rows are not migrated. Browser downloads serialize the artifact to a JSON Blob, activate a same-document download anchor, and revoke the Blob URL after activation.
...
`window.__SUITE__` exposes readiness, environment/support, registration details, complete registered ids, and a filtered `run()` method. The launcher waits for readiness or boot error, validates the browser-side inventory, forwards engine/feature/operation/scenario/pillar/timing/exhaustive filters, polls completion, and snapshots partial results every fifteen seconds.
...
The launcher opens a visible Playwright browser. Both `launch.mjs` and `run.sh` accept and forward `--exhaustive`; canonical selection requires no replay parameter.
...
3. Warmup `0`, iteration bounds, browser tag, reuse policy, exhaustive mode, feature/scenario filters, and operation/pillar filters round-trip through a visible run configuration summary and the export manifest.
...
5. Media files remain catalog-controlled by the [media-selection subsystem](../subsystems/media-selection.md); the UI shows the selected input variant(s), candidate count, and SHA-256 after selection rather than introducing an untracked local-file path.
...
Every run must have a visible and exported immutable manifest: schema/status-model version, run id, start/end time, completion state, suite/build revision, selected engine instance ids and configurations, detected browser/build plus operator tag, user agent, GPU, capability snapshot, scenario/oracle-definition digest, filters, warmup/iterations, deterministic execution-order digest, media mode, corpus checksum, selected files and hashes, cache policy/hits, and registration failures. Current per-result `env` and `selection` fields can supply part of this record; the UI must surface them instead of leaving them only in JSON.
...
The command interface must have option parity: `run.sh`, `launch.mjs`, visible UI, and exported filter schema all support exhaustive mode, reuse/fresh policy, filters, timing, timeout, and browser choice with one meaning.
...
- **Current:** IndexedDB is origin-scoped and cross-port reuse requires explicit validated import.
- **Consequence:** A persistent browser profile alone does not share rows across origins.
...
- **Current:** Manual export omits the active filter, Playwright version, completion/partial state, registration report, and report artifacts; launcher export adds only some of those fields. [src/app/main.ts:393-405](../../src/app/main.ts#L393-L405) [scripts/launch.mjs:277-305](../../scripts/launch.mjs#L277-L305)
...
- **Current:** Playwright opens a visible browser and both wrappers accept the canonical exhaustive/reuse options.
- **Consequence:** Help and accepted options must remain synchronized.
...
- [src/app/main.ts](../../src/app/main.ts) — run state initialization, filters, display ids, deterministic matrix, and callbacks.
...
- [src/core/media-selection.ts](../../src/core/media-selection.ts) — canonical/exhaustive candidates and byte-sensitive cache tag.
...
- [scripts/launch.mjs](../../scripts/launch.mjs) — visible persistent context and page boot.
