...
For a rotatable row, the shape gate lowercases and exactly compares the container, requires every declared input codec token to occur in the corresponding file array, rejects video on an audio-only requirement, and checks a required encryption scheme against `file.keys.scheme`.[src/core/media-selection.ts:173-200](../../src/core/media-selection.ts#L173-L200) Trim additionally requires a finite catalog duration at least 20 ms beyond the larger requested boundary; the same helper can derive a seek target, although seek is already baked-only.[src/core/media-selection.ts:201-230](../../src/core/media-selection.ts#L201-L230) A rejected real file is omitted and produces a shape warning; if no real survives, the baked candidate remains.[src/core/media-selection.ts:374-390](../../src/core/media-selection.ts#L374-L390)

The gate does not inspect profile, level, bit depth, dimensions, frame cadence, channel layout, track count, or other option-conditioned traits. It also does not require a video stream when `video` is true unless a required video codec token supplies that constraint; these omissions follow from the fields actually read by the gate.[src/core/media-selection.ts:173-210](../../src/core/media-selection.ts#L173-L210)

### Seeded single-file rotation and exhaustive enumeration

In seeded-single mode, selection iterates scenarios once. For each scenario it hashes the string `runSeed|scenarioId` with 32-bit FNV-1a, initializes `mulberry32`, draws one number where `0 ≤ draw < 1`, and chooses `floor(draw × candidateCount)`. The candidate order is baked first followed by shape-passing real files in catalog order. [src/core/media-selection.ts:397-413](../../src/core/media-selection.ts#L397-L413) [src/core/seeded-rng.ts:10-30](../../src/core/seeded-rng.ts#L10-L30) The same seed and unchanged ordered catalog therefore reproduce a pick; the unit script checks repeated-seed equality and confirms that baked and real candidates are reachable over a seed sweep. [scripts/test-media-selection.mjs:51-93](../../scripts/test-media-selection.mjs#L51-L93)

In exhaustive mode, the selector returns every gathered candidate in that same stable order and does not use the RNG.[src/core/media-selection.ts:416-435](../../src/core/media-selection.ts#L416-L435) The runner computes both the single selections and exhaustive lists once before iterating engine × scenario cells, so all engines share the same per-scenario pick or candidate set.[src/core/runner.ts:1784-1803](../../src/core/runner.ts#L1784-L1803) The browser UI currently checks exhaustive mode by default, while the launcher enables it only through `--exhaustive`; the app creates a timestamp-plus-random seed when none is supplied.[src/app/main.ts:228-250](../../src/app/main.ts#L228-L250) [src/app/main.ts:309-332](../../src/app/main.ts#L309-L332) [scripts/launch.mjs:48-70](../../scripts/launch.mjs#L48-L70)
...
The effective scenario preserves its registered id, family, and requirements; only its input and, for derived rotation, options/oracles differ.[src/core/media-selection.ts:83-103](../../src/core/media-selection.ts#L83-L103) Consequently the public cell remains engine × registered scenario × browser. The selected input variant is additional identity: `runOne` stamps file, optional SHA-256, baked flag, seed, and candidate count onto `ScenarioResult.selection`.[src/core/runner.ts:1258-1279](../../src/core/runner.ts#L1258-L1279) The result model also provides selection provenance, an exhaustive per-file array, and `passed/admissible/total` coverage counts.[src/core/scenario.ts:287-349](../../src/core/scenario.ts#L287-L349)
...
The report projects file, SHA-256, baked flag, candidate count, and exhaustive per-file status into conformance cells, but intentionally drops the run seed and does not project each sub-result's reason, bench, or oracle outcomes.[src/core/report.ts:52-70](../../src/core/report.ts#L52-L70) [src/core/report.ts:541-555](../../src/core/report.ts#L541-L555) It detects merged run checksums and warns against comparing across them.[src/core/report.ts:317-343](../../src/core/report.ts#L317-L343)
...
- **Baked fallback is structurally nonempty.** `gatherCandidates` initializes the pool with baked and only replaces it with baked plus surviving real files, so a successfully gathered scenario never asks the RNG to choose from an empty array.[src/core/media-selection.ts:352-390](../../src/core/media-selection.ts#L352-L390) Whether the baked bytes actually exist is checked later by the runner, not by the selector.[src/core/runner.ts:1296-1323](../../src/core/runner.ts#L1296-L1323)
- **Seed replay is conditional on the ordered corpus.** The implemented replay key is the seed and scenario id, but the index is interpreted against catalog order and candidate count. Same seed alone is not an immutable input variant identity.[src/core/media-selection.ts:397-413](../../src/core/media-selection.ts#L397-L413)
...
#### 2. Make seeded selection unbiased, stable, and replayable

Within each scenario, every unique eligible content candidate must have the same documented selection probability unless an explicit, reported weighting policy says otherwise. NIST defines random sampling as giving each sample an equal chance in pursuit of an unbiased representation.[NIST, “random sampling”](https://csrc.nist.gov/glossary/term/random_sampling) Provider duplication, repeated paths, or byte-identical files therefore cannot silently weight a source. Any future strata or weights must be manifest fields and must be applied identically for every engine.

Replace catalog-index selection with an order-independent keyed score over `selectionPolicyVersion | seed | scenarioId | fullCandidateDigest`, choosing the highest score with a canonical tie-break. Highest-random-weight mapping is a published name-based selection method that maps a key consistently across a candidate set and limits churn when membership changes.[Thaler and Ravishankar, “Using Name-Based Mappings to Increase Hit Rates”](https://doi.org/10.1109/90.663936) Use a specified cryptographic hash-to-integer construction; do not rely on unspecified object iteration or floating-point multiplication.

Record the seed, selection algorithm id/version, exact selected logical path and full SHA-256, eligible pool digest, and candidate count. A seed is a convenient experiment key, not the primary replay artifact: Hypothesis documents that fixed-seed example reproduction assumes no other nondeterminism, while its replay guidance recommends retaining explicit failing inputs for durable regression coverage.[Hypothesis `seed`](https://hypothesis.readthedocs.io/en/latest/reference/api.html#hypothesis.seed) [Hypothesis, “Replaying failed tests”](https://hypothesis.readthedocs.io/en/latest/tutorial/replaying-failures.html) Replay by recorded path plus digest must work even if the live catalog later changes; replay by seed must reject a pool-digest mismatch unless the caller explicitly opts into a new selection.

Version the algorithm and preserve its implementation or migration fixture. NumPy's RNG compatibility policy makes the dependency explicit: stream replay requires the same bit generator, seed, call sequence, build, and environment, and defaults may change.[NumPy, “Compatibility policy”](https://numpy.org/doc/stable/reference/random/compatibility.html) This selector can provide a stronger local contract by using only specified integer/hash operations and by recording the policy version.

Acceptance criteria:

1. Same policy version, seed, scenario id, and eligible-pool digest select the same full digest in every supported browser.
2. Reordering catalog rows or file entries never changes the pick.
3. Adding or removing a candidate changes a prior pick only when the set-scoring rule selects the changed member; unchanged candidates retain their scores.
4. Across a large deterministic seed sweep, unique candidates meet a predeclared uniformity test, and a duplicate-content fixture is rejected rather than double-weighted.
5. A recorded failing digest is directly replayable as an explicit input, independent of random selection.
...
5. Reusing identical bytes under a new seed is allowed only if the result is stamped with the new run envelope while retaining a trace to the original observation.

#### 6. Test the policy as a property, not only as examples

Keep focused example tests for baked-only families, shape/duration boundaries, id/URL separation, and CENC rewrite. Add property tests over generated row permutations, candidate additions/removals, duplicate content, malformed fields, empty pools, and seed sweeps. Property-based testing explores a declared input range and retains counterexamples; Hypothesis's documented replay model is a useful benchmark for the required “record the exact failing input” behavior.[Hypothesis documentation](https://hypothesis.readthedocs.io/en/latest/)

The test suite must cover `candidatesForRun`, integration with `runMatrix`, cache hits, and report serialization—not just `selectForRun`. Include a fixed golden vector for each `selectionPolicyVersion` so later algorithm refactors either preserve the stream or intentionally introduce a new version.
...
#### Fixed seed does not identify a stable sample by itself

**Current.** Selection hashes seed plus scenario and indexes catalog order; neither the RNG/policy version nor the eligible-pool digest is recorded with the result.[src/core/media-selection.ts:397-413](../../src/core/media-selection.ts#L397-L413) [src/core/scenario.ts:338-349](../../src/core/scenario.ts#L338-L349)

**Consequence.** Reordering or inserting files can remap a seed, and the same textual seed can mean different bytes across corpus revisions. NumPy similarly limits stream reproducibility to an explicitly fixed generator and execution context.[NumPy, “Compatibility policy”](https://numpy.org/doc/stable/reference/random/compatibility.html)

**Target.** Use order-independent candidate scoring, version the policy, record the pool digest, and make full path plus full SHA-256 the durable replay key.

**Verification.** Cross-browser vectors survive row permutations; pool drift is detected; an explicitly recorded failed digest replays even after unrelated candidates are added.

#### Sampling probability can be accidentally weighted

**Current.** Every file entry occupies one array slot, with no duplicate-content detection or stratification metadata; baked receives one slot regardless of corpus size.[src/core/media-selection.ts:375-410](../../src/core/media-selection.ts#L375-L410)

**Consequence.** Duplicate downloads or a provider with many near-identical files receive extra probability, baked coverage decays as reals are appended, and equal slot probability need not represent the intended media population. NIST's equal-chance definition only guarantees unbiasedness relative to the population as actually enumerated.[NIST, “random sampling”](https://csrc.nist.gov/glossary/term/random_sampling)

**Target.** Define the sampling unit as unique verified content, disclose probabilities, and introduce explicit weights/strata only with a versioned rationale and report fields.

**Verification.** Duplicate-content fixtures do not change probability; a seed sweep meets the declared distribution; reports enumerate the pool and weights used.
...
**Verification.** Baked mutation, prefix collision, pool-only drift, oracle-contract change, and same-bytes/new-seed tests each demonstrate the intended hit/miss and current-envelope behavior.

#### Current tests stop before exhaustive integration

**Current.** The unit script covers deterministic single selection, basic reachability, shape and duration gates, baked-only policies, id/URL behavior, CENC rewrite, cache tags, checksum order independence, seek exclusion, and fetch fallback.[scripts/test-media-selection.mjs:51-130](../../scripts/test-media-selection.mjs#L51-L130) [scripts/test-media-selection.mjs:134-227](../../scripts/test-media-selection.mjs#L134-L227) [scripts/test-media-selection.mjs:230-367](../../scripts/test-media-selection.mjs#L230-L367) It imports neither `candidatesForRun` nor runner/report integration.[scripts/test-media-selection.mjs:15-21](../../scripts/test-media-selection.mjs#L15-L21)
...
**Verification.** CI exercises seeded-single and exhaustive modes through report JSON and proves the acceptance matrix for PASS/DIFF/FAIL/NA, denominators, failing identities, cache behavior, and manifest drift.
...
- [src/core/media-selection.ts:397-435](../../src/core/media-selection.ts#L397-L435) — seeded-single draw and exhaustive enumeration.
- [src/core/media-selection.ts:438-454](../../src/core/media-selection.ts#L438-L454) — cache tags and 32-bit corpus checksum.
- [src/core/seeded-rng.ts:10-30](../../src/core/seeded-rng.ts#L10-L30) — FNV-1a seed hash and mulberry32 stream.
...
- [src/app/main.ts:309-332](../../src/app/main.ts#L309-L332) — generated run seed and runner options.
- [src/app/result-cache.ts:43-50](../../src/app/result-cache.ts#L43-L50) — IndexedDB result-key components and epoch invalidation.
- [scripts/launch.mjs:48-70](../../scripts/launch.mjs#L48-L70) — launcher seed and exhaustive flags.
...
- National Institute of Standards and Technology, “random sampling,” CSRC Glossary (source: NIST SP 800-55 Vol. 1 / Handbook 151), [definition](https://csrc.nist.gov/glossary/term/random_sampling), accessed 2026-07-16 — supports equal selection opportunity as the baseline for an unbiased enumerated population.
...
- David G. Thaler and Chinya V. Ravishankar, “Using Name-Based Mappings to Increase Hit Rates,” *IEEE/ACM Transactions on Networking* 6(1), 1998, [doi:10.1109/90.663936](https://doi.org/10.1109/90.663936), accessed 2026-07-16 — supplies highest-random-weight mapping as an order-independent, set-based deterministic choice technique.
- NumPy developers, “Compatibility policy,” *NumPy Random Sampling Reference*, [official documentation](https://numpy.org/doc/stable/reference/random/compatibility.html), accessed 2026-07-16 — documents the generator, seed, call sequence, build, and environment conditions required for stream compatibility.
- Hypothesis maintainers, “Reproducing inputs” (`hypothesis.seed`) and “Replaying failed tests,” [API reference](https://hypothesis.readthedocs.io/en/latest/reference/api.html#reproducing-inputs) and [tutorial](https://hypothesis.readthedocs.io/en/latest/tutorial/replaying-failures.html), accessed 2026-07-16 — supports conditional seed reproducibility and durable retention of explicit failing examples.
