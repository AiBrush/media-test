...
echo "  3. Add adapter fixtures for every declared op: positive, negative tuple, lifecycle,"
echo "     normalized result, and cancellation. Keep malformed input as an ordinary rejection."
echo "  4. REQUIRED PRE-WIRING GATE: bun test tests/adapter-contract-conformance.test.ts"
echo "  5. Only after that gate passes, wire the engine into src/app/register.ts importing"
echo "     '../engines/${ID}/adapter.ts' and calling register${PASCAL}()."
echo "  6. Serve + run:  bash scripts/serve.sh   then   bash scripts/run.sh --engine ${ID}@<version>"
echo "  7. Compare:      bash scripts/compare.sh   → results/report.md (Δ vs the reference engine)."
echo ""
