...
export const SCENARIO_DEFINITION_V2_SCHEMA = {
	...
	properties: {
		...
		mutation: {
			type: "object",
			additionalProperties: false,
			required: ["mutationId", "parameters"],
			properties: {
				mutationId: {
					type: "string",
					pattern: "^[A-Za-z0-9][A-Za-z0-9._:/-]*$",
	...
	$defs: {
		...
		candidateEnvelope: {
			...
			properties: {
				...
				minAudioMeanVolumeDb: { type: "number", minimum: -160, maximum: 24 },
				excludeDimensions: {
					type: "array",
					minItems: 1,
					uniqueItems: true,
		...
		tokens: {
			...
			properties: {
				...
				containersIn: { $ref: "#/$defs/stringTokens" },
				containersOut: { $ref: "#/$defs/stringTokens" },
				videoCodecs: { $ref: "#/$defs/stringTokens" },
				audioCodecs: { $ref: "#/$defs/stringTokens" },
				videoCodecsIn: { $ref: "#/$defs/stringTokens" },
				audioCodecsIn: { $ref: "#/$defs/stringTokens" },
				videoCodecsOut: { $ref: "#/$defs/stringTokens" },
				audioCodecsOut: { $ref: "#/$defs/stringTokens" },
				encryption: { $ref: "#/$defs/stringTokens" },
				features: { $ref: "#/$defs/stringTokens" },
			},
		},
		combination: {
			type: "object",
...
export const RESULTS_V2_JSON_SCHEMA = {
	...
	properties: {
		...
		migration: {
			type: "object",
			additionalProperties: false,
			required: ["from", "missingInputDigests"],
			properties: {
	...
	$defs: {
		...
		unavailableOutcome: {
			...
			properties: {
				...
				oracle: { $ref: "#/$defs/oracleId" },
				status: { enum: ["NA_BROWSER", "NA_ASSET"] },
				reasonCode: { type: "string", minLength: 1 },
				detail: { type: "string" },
				measurements: { $ref: "#/$defs/measurements" },
				evidence: { type: "object" },
		...
		instance: {
			...
			properties: {
				...
				definitionHash: {
					type: "string",
					pattern: "^(?:[a-f0-9]{64}|legacy/[a-f0-9]{64})$",
				},
				inputVariantId: { type: "string", minLength: 1 },
		...
		operationEvidence: {
			type: "object",
			additionalProperties: false,
			required: ["schema", "disposition", "stage"],
			properties: {
				...
				stage: {
					enum: ["preflight", "operation", "survivor-oracle", "cleanup"],
				},
				nativeError: {
					type: "object",
					additionalProperties: false,
				...
				resource: {
					...
					required: ["kind"],
					properties: {
						kind: { enum: ["wall-time", "memory", "worker-stall"] },
			...
			properties: {
				...
				unavailable: {
					...
					items: {
						type: "object",
						additionalProperties: false,
						required: ["oracle", "status", "reasonCode"],
						properties: {
							oracle: { $ref: "#/$defs/oracleId" },
		...
		cacheReuse: {
			...
			properties: {
				schema: { const: "media-test/cache-reuse@1" },
				sourceKey: { type: "string", minLength: 1 },
				sourceObservationHash: { $ref: "#/$defs/sha256" },
				sourceRunId: { type: "string", minLength: 1 },
				createdAtIso: { type: "string", format: "date-time" },
				originalOrigin: { type: "string", minLength: 1 },
				validationEpoch: { type: "string", minLength: 1 },
				validBecause: { type: "string", minLength: 1 },
				importedFrom: { type: "string", minLength: 1 },
				sourceEnvironment: { $ref: "#/$defs/runEnv" },
		...
		fingerprint: {
			...
			properties: {
				schema: { const: "media-test/scenario-result@3" },
		...
		bench: {
			type: "object",
			propertyNames: {
				$ref: "scenario-definition-v2.schema.json#/$defs/metric",
			},
		...
		measurement: {
			oneOf: [
				...
				},
				{
					type: "object",
					additionalProperties: false,
					required: ["state", "reasonCode", "detail"],
					properties: {
						state: { const: "UNAVAILABLE" },
						reasonCode: { type: "string", minLength: 1 },
		...
		pixelBehavior: {
			...
			required: ["state", "reasonCode", "detail"],
			properties: {
				state: { enum: ["SUPPORTED", "UNSUPPORTED"] },
				reasonCode: { type: "string", minLength: 1 },
				detail: { type: "string", minLength: 1 },
			},
		},
		runEnv: {
			type: "object",
			additionalProperties: false,
			required: ["suiteVersion", "engineId", "browser"],
			properties: {
		...
		exhaustive: {
			...
			properties: {
				...
				oracleOutcomes: { type: "array", items: { $ref: "#/$defs/outcome" } },
				reason: { type: "string" },
				bench: { $ref: "#/$defs/bench" },
				measurement: { $ref: "#/$defs/measurement" },
				support: {},
				selection: { $ref: "#/$defs/selection" },
				operationEvidence: { $ref: "#/$defs/operationEvidence" },
		...
		coverage: {
			...
			properties: {
				...
				admissible: { type: "integer", minimum: 0 },
		...
		result: {
			...
			properties: {
				...
				family: { $ref: "scenario-definition-v2.schema.json#/$defs/family" },
				status: { $ref: "#/$defs/status" },
				oracleOutcomes: { type: "array", items: { $ref: "#/$defs/outcome" } },
				inputVariantId: { type: "string", minLength: 1 },
