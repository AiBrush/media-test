...
export class LoadedScenarioSources extends Map<string, ScenarioSourceRow> {
	...
	constructor(
		entries: readonly (readonly [string, ScenarioSourceRow])[],
		metadata: LoadedCatalogMetadata,
	) {
		super();
...
export async function loadScenarioSources(
	...
	const [catalogFetch, bakedFetch] = await Promise.allSettled([
		fetch(url, { cache: "no-store" }),
		fetch(bakedUrl, { cache: "no-store" }),
	]);
	...
	if (bakedFetch.status === "fulfilled" && bakedFetch.value.ok) {
		try {
			...
			issues.push(
				loadIssue("BAKED_MANIFEST_PARSE_FAILED", errorMessage(error)),
			);
		}
	} else {
		const detail =
			bakedFetch.status === "rejected"
	...
	if (catalogFetch.status === "rejected") {
		fallbackReason = {
			reasonCode: "CATALOG_FETCH_FAILED",
			detail: errorMessage(catalogFetch.reason),
		};
			...
			if (parsed.state === "VALID") catalog = parsed.catalog;
				...
				fallbackReason = {
					...
					detail: parsed.issues
						.map((entry) => `[${entry.reasonCode}] ${entry.detail}`)
						.join("; "),
				};
			}
		} catch (error) {
			fallbackReason = {
				reasonCode: "CATALOG_PARSE_FAILED",
	...
	if (!catalog) {
		const reason = fallbackReason ?? {
			...
			detail: "selection catalog unavailable",
		};
		console.warn(
			`media-selection: baked-only fallback [${reason.reasonCode}] ${reason.detail}`,
		);
...
export function buildSelectionManifest(
	options: BuildSelectionManifestOptions,
): FrozenSelectionManifest {
	const catalogState: "ready" | "fallback" = options.catalog
		? "ready"
		: "fallback";
	if (!options.catalog && !options.catalogFallbackReason) {
		throw new SelectionPolicyError(
			"CATALOG_FALLBACK_REASON_REQUIRED",
			"baked-only fallback requires a structured reason",
		);
	}
	const sourceMap = options.catalog
		? scenarioSourceMap(options.catalog)
		: new Map<string, ScenarioSourceRow>();
	const bakedMap = new Map(
		options.bakedManifest.assets.map((asset) => [asset.id, asset] as const),
	);
	const pools = [...options.scenarios]
		.sort((a, b) => a.id.localeCompare(b.id))
		.map((scenario) =>
			buildScenarioPool(scenario, sourceMap.get(scenario.id), bakedMap, {
				rotate: options.rotate,
				catalogState,
				scenarioContractDigest: contractDigestFor(
					options.scenarioContractDigests,
					scenario.id,
				),
			}),
		);
	const identity = {
		...
		catalogState,
		...(options.catalogFallbackReason
			? { catalogReason: options.catalogFallbackReason }
			: {}),
		...(options.catalog
...
function buildScenarioPool(
	...
	if (policyReason) {
		...
		const seenDigests = new Set(
			bases.map((candidate) => candidate.contentDigest),
		);
		for (const file of row.files) {
			...
			bases.push(
				createRealCandidate(scenario, row, file, eligibility.evidencePlan),
			);
		}
	}
	...
	const candidates = sortedBases.map((candidate) => ({
		...
		probability: {
			numerator: 1,
			denominator: sortedBases.length,
			weight: 1,
		} as const,
	}));
	const eligiblePoolDigest = computeEligiblePoolDigest(
		scenario.id,
		candidates,
...
function createBakedCandidate(
		...
		if (!asset) {
			rejections.push(
				rejection(
					scenario.id,
					name,
					"BAKED_ASSET_IDENTITY_MISSING",
					`baked manifest has no verified '${name}'`,
				),
			);
			return undefined;
		}
	...
	const contentDigest =
		inputs.length === 1 ? inputs[0]!.sha256 : contentIdentityDigest(inputs);
...
function createUnverifiedBakedCandidate(
	scenario: Scenario,
...
function createRealCandidate(
	...
	evidencePlan = buildCandidateEvidencePlan(
		scenario,
		file.sha256,
		file.evidence,
	),
...
function rotationPolicyReason(
	...
	if (options.rotate === false)
		return { reasonCode: "ROTATION_DISABLED", detail: "rotation is disabled" };
	if (options.catalogState === "fallback")
		return {
			reasonCode: "CATALOG_FALLBACK_BAKED_ONLY",
			detail: "catalog is in baked-only fallback",
		};
	if (!row)
		return {
			reasonCode: "CATALOG_SCENARIO_ROW_MISSING",
			detail: "scenario has no catalog row",
		};
	if (row.class === "SYNTHETIC")
		return {
			reasonCode: "SYNTHETIC_ROW_BAKED_ONLY",
			detail: "synthetic media is constructed by the fixture contract",
		};
	if (row.class === "STREAMING")
		return {
			reasonCode: "STREAMING_ROW_BAKED_ONLY",
			detail: "stream graphs are fixture-bound",
		};
	if (row.files.length === 0)
		return {
			reasonCode: "CATALOG_ROW_EMPTY",
			detail: "catalog row has no candidates",
		};
	if (scenario.family === "streaming-output")
		return {
			reasonCode: "STREAMING_OUTPUT_BAKED_ONLY",
			detail: "streaming-output topology is fixture-bound",
		};
	if (Array.isArray(scenario.input))
		return {
			reasonCode: "MULTI_INPUT_BAKED_ONLY",
			detail: "no catalog bundle declares this ordered multi-input contract",
		};
	if (scenario.op === "seek")
		return {
			reasonCode: "SEEK_TARGET_FIXTURE_BOUND",
			detail: "seek target is calibrated to the baked input",
		};
	if (scenario.id === "performance/decode-fps") {
		return {
			reasonCode: "DECODE_GOLDEN_PREFIX_FIXTURE_BOUND",
			detail:
				"the decoded-frame benchmark is calibrated to the baked input and its committed RGBA prefix",
		};
	}
	if (FIXTURE_BOUND_INPUT_SEMANTIC_SCENARIOS.has(scenario.id)) {
		return {
			...
			detail:
				"the authored input semantics are carried by the baked fixture and are not declared by the catalog row",
		};
	}
	if (AUDIO_DSP_FIXED_INPUT_SCENARIOS.has(scenario.id)) {
		return {
			...
			detail:
				"the authored sample-rate, channel-layout, or gapless contract is calibrated to the baked input",
		};
	}
	if (row.class === "DERIVED" && !isRotatableCencMp4(row)) {
		return {
			reasonCode: "DERIVED_SCHEME_NOT_ROTATABLE",
			detail: "derived row is not supported CENC-in-MP4",
		};
	}
...
export function assessCandidateEligibility(
	...
	if (scenario.family === "robustness") {
		return assessRobustnessVariantEligibility(
			scenario,
			file,
...
export function assessRobustnessVariantEligibility(
	...
	if (!scenarioContractDigest || !LOWER_SHA256.test(scenarioContractDigest)) {
		return reject(
			"ROBUSTNESS_SCENARIO_CONTRACT_MISSING",
			"runner/DSL did not supply a full scenario contract digest",
		);
	}
	const contract = file.contract;
	if (!contract)
		return reject(
			"ROBUSTNESS_VARIANT_CONTRACT_MISSING",
			"candidate has no same-contract declaration",
		);
	if (
		...
		return reject(
			"ROBUSTNESS_VARIANT_CONTRACT_MISMATCH",
			"candidate contract is not digest-bound to this scenario and source",
		);
	}
	if (!file.evidence || file.evidence.sourceSha256 !== file.sha256) {
		return reject(
			"ROBUSTNESS_VARIANT_EVIDENCE_MISSING",
			"candidate has no source-bound evidence declaration",
		);
	}
	const evidencePlan = buildCandidateEvidencePlan(
		scenario,
		file.sha256,
		file.evidence,
	);
		...
		set.every(
			(oracle) =>
				evidencePlan.requirements
					.find((requirement) => requirement.oracle === oracle)
					?.needs.every((need) => available.has(need.kind)) === true,
		),
	);
	if (!sufficientDeclared) {
		return reject(
			"ROBUSTNESS_VARIANT_EVIDENCE_INSUFFICIENT",
			"declared evidence cannot satisfy any survivor set",
		);
	}
	return { eligible: true, evidencePlan };
}

function assessDerivedCencEligibility(
	scenario: Scenario,
	file: SourceFileRecord,
): CandidateEligibility {
	const reject = (
	...
	if (
		...
		return reject(
			"CENC_KEY_MATERIAL_INCOMPLETE",
			"CENC candidate needs scheme, keyHex, and kid",
		);
	}
	const base = file.cleartextBase;
	if (
		!base ||
		normalizeRelativePath(base.poolPath) !== base.poolPath ||
		!LOWER_SHA256.test(base.sha256)
	) {
		return reject(
			"CENC_CLEARTEXT_BASE_INCOMPLETE",
			"CENC candidate needs a normalized base path and full base digest",
		);
	}
	if (isProtectedProbeCencScenario(scenario)) {
		if (
			!(scenario.requires.encryption ?? []).some(
				(scheme) => scheme === keys.scheme,
			)
		) {
			return reject(
		...
		if (
			...
			!declaration.sufficientOracleSets.some(
				(set) => set.length === 1 && set[0] === "golden-metadata",
			)
		) {
			return reject(
	...
	if (rotation.state === "INELIGIBLE")
		return reject(rotation.reasonCode, rotation.detail);
	const declaration = file.evidence;
	...
	if (
		...
		!declaration.sufficientOracleSets.some(
			(set) => set.length === 1 && set[0] === "property-invariant",
		)
	) {
		return reject(
			"CENC_METAMORPHIC_EVIDENCE_INCOMPLETE",
			"CENC candidate lacks its own source/base-bound survivor invariant",
		);
	}
	return {
		eligible: true,
...
export function buildCandidateEvidencePlan(
	...
	const declaredOracles = forcedOracles
		? [...forcedOracles]
		: [...scenario.oracles];
	...
	const conditionalGracefulFailure =
		!forcedOracles &&
		declaredSet.has("graceful-failure") &&
		declaredOracles.length > 1;
	const survivorOracles = declaredOracles.filter(
		(oracle) => oracle !== "graceful-failure",
	);
	...
	const sufficientOracleSets = declaration?.sufficientOracleSets?.length
		? declaration.sufficientOracleSets
				.map((set) =>
					[...new Set(set.filter((oracle) => declaredSet.has(oracle)))].sort(),
				)
	...
	const requirements = declaredOracles
		.map(
			(oracle): OracleEvidenceRequirement => ({
				oracle,
...
function oracleEvidenceNeeds(
	...
	if (oracle === "mp4-box-layout" || oracle === "webm-live-layout")
		return [{ kind: "STRUCTURAL_OUTPUT" }];
	if (oracle === "graceful-failure") return [{ kind: "MALFORMED_REJECTION" }];
	if (oracle === "property-invariant") {
		return [
			{
				kind: "METAMORPHIC_PEER",
				sourceSha256,
				...(baseSha ? { peerSha256: baseSha } : {}),
			},
		];
	}
	if (GOLDEN_ORACLES.has(oracle)) {
		const needs: OracleEvidenceNeed[] = [
			{ kind: "SOURCE_GOLDEN", sourceSha256 },
		];
		if (
...
export function evaluateCandidateEvidence(
	...
	const byOracle = new Map(
		outcomes.map((outcome) => [outcome.oracle, outcome] as const),
	);
	const applied = plan.requirements
		.filter(
			(requirement) => byOracle.get(requirement.oracle)?.state === "VERDICT",
		)
	...
	const verdictOutcomes = outcomes.filter(
		(outcome) => outcome.state === "VERDICT",
	);
	...
	if (verdictOutcomes.some((outcome) => outcome.verdict === "FAIL")) {
		return deepFreeze({
			...base,
			status: "FAIL",
			reasonCode: "EVIDENCE_VERDICT_FAIL",
		});
	}
	if (outcomes.some((outcome) => outcome.state === "ERROR")) {
		return deepFreeze({
			...base,
			status: "ERROR",
			reasonCode: "EVIDENCE_ORACLE_ERROR",
		});
	}
	if (!sufficientSet) {
		const status =
			unavailable.length > 0 &&
			unavailable.every((entry) => entry.status === "NA_BROWSER")
				? "NA_BROWSER"
				: "NA_ASSET";
		return deepFreeze({
			...base,
...
export function candidateReplayKey(
	candidate: SelectionCandidateManifest,
): CandidateReplayKey {
	return deepFreeze({
		scenarioId: candidate.scenarioId,
		candidateIdentity: candidate.candidateIdentity,
		inputs: candidate.inputs.map(({ logicalPath, sha256, sizeBytes }) => ({
			logicalPath,
			sha256,
			sizeBytes,
		})),
...
export function selectCandidateFromPool(
	...
	const selected = pool.candidates.reduce((current, candidate) =>
		candidate.candidateIdentity < current.candidateIdentity
			? candidate
			: current,
	);
	return deepFreeze({
...
export function computeEligiblePoolDigest(
	...
	candidates: readonly Pick<
		SelectionCandidateManifest,
		"candidateIdentity" | "contentDigest" | "probability"
	>[],
...
export function findScenarioPool(
	manifest: FrozenSelectionManifest,
	scenarioId: string,
): ScenarioCandidatePool | undefined {
...
export interface SelectOptions {
	...
	allowPoolDrift?: boolean;
	replayCandidates?:
		| ReadonlyMap<string, SelectionCandidateManifest>
		| Readonly<Record<string, SelectionCandidateManifest>>;
}
...
export function selectForRun(
	...
	for (const scenario of scenarios) {
		...
		if (decision.state === "POOL_MISMATCH") {
			throw new SelectionPolicyError(
				decision.reasonCode,
				`expected ${decision.expectedPoolDigest}, got ${decision.actualPoolDigest}`,
			);
		}
		if (decision.state === "EMPTY") continue;
		out.set(
			scenario.id,
			makeSelection(scenario, decision.candidate, pool, sources),
		);
	}
	return out;
}
	...
	for (const scenario of scenarios) {
		...
		out.set(
			...
			pool.candidates.map((candidate) =>
				makeSelection(scenario, candidate, pool, sources),
			),
		);
	}
	return out;
...
function compatibilityPool(
	...
	if (loaded) {
		...
		return buildScenarioPool(scenario, sources.get(scenario.id), new Map(), {
			rotate: options.rotate,
	...
	const rejections: CandidateRejection[] = [];
	const bases: Array<Omit<SelectionCandidateManifest, "probability">> = [
		createUnverifiedBakedCandidate(scenario),
	];
			...
			if (!LOWER_SHA256.test(file.sha256)) {
				rejections.push(
					rejection(
						...
						"candidate digest must be full lowercase SHA-256",
					),
				);
				continue;
			}
			if (seen.has(file.sha256)) {
				rejections.push(
					rejection(
						...
						file.file,
						"CANDIDATE_DUPLICATE_CONTENT",
						"duplicate content is one sampling unit",
					),
				);
				continue;
			}
	...
	return deepFreeze({
		scenarioId: scenario.id,
		eligiblePoolDigest: computeEligiblePoolDigest(
			scenario.id,
			candidates,
			scenario.candidateEnvelope,
		),
...
function makeSelection(
	...
	const resolvedInputs = candidate.inputs.map(
		(input, index): ResolvedInput => ({
			id: input.id,
	...
	const effectiveScenario = isBaked
		...
		: candidate.sourceClass === "DERIVED" && candidate.sourceFile
			? isProtectedProbeCencScenario(scenario)
				...
				: deriveCencEffectiveScenario(
						scenario,
						candidate.inputs[0]!.id,
	...
	return {
		...
		shapeWarnings: pool.rejections
			.filter(
				(entry) => entry.reasonCode === "CANDIDATE_INPUT_CONTRACT_MISMATCH",
			)
			.map(
				(entry) =>
					`${entry.scenarioId}: dropped ${entry.selectedFile ?? "?"} (${entry.detail})`,
			),
		eligiblePoolDigest: pool.eligiblePoolDigest,
		...
		...(sources instanceof LoadedScenarioSources
			? {
					...
					...(sources.metadata.catalogReason
						? { catalogReason: sources.metadata.catalogReason }
						: {}),
				}
			: {}),
	};
...
function deriveCencEffectiveScenario(
	scenario: Scenario,
	id: string,
	file: SourceFileRecord,
): Scenario {
	...
	const options: Record<string, unknown> = {
		...
		cleartextBaseSha256: base.sha256,
		...(base.sizeBytes !== undefined
			? { cleartextBaseSizeBytes: base.sizeBytes }
			: {}),
...
export function exhaustiveSelectionCacheTag(
	...
	return `set-sha256:${canonicalSha256({
		...
		inputs: selections
			.map(
				(selection) =>
					selection.executedInputDigest ??
					selectionCacheTag(selection).slice("sha256:".length),
			)
			.sort(),
	})}`;
}
...
export function computeCorpusChecksum(
	...
	rows.sort(
		(a, b) =>
			...
			a.executedInputDigest.localeCompare(b.executedInputDigest),
	);
	return canonicalSha256({ schema: "executed-corpus@1", rows });
}

export function computeEligiblePoolsDigest(
	selections: Iterable<ScenarioSelection>,
): string {
	const pools = [...selections]
		.map((selection) => ({
			scenarioId: selection.scenarioId,
			eligiblePoolDigest: selection.eligiblePoolDigest ?? "",
		}))
...
function shapeGateReason(
	...
	for (const codec of requires.videoCodecs) {
		if (!file.videoCodecs.includes(codec))
			return `missing video codec '${codec}'`;
	}
	for (const codec of requires.audioCodecs) {
		if (!file.audioCodecs.includes(codec))
			return `missing audio codec '${codec}'`;
	}
	if (
		!requires.video &&
		requires.audioCodecs.length > 0 &&
		file.videoCodecs.length > 0
	) {
	...
	const scenarioEnvelopeReason = sourceEnvelopeGateReason(
		file,
		scenarioEnvelope,
	);
	if (scenarioEnvelopeReason)
		return `scenario candidate envelope: ${scenarioEnvelopeReason}`;
	const catalogEnvelopeReason = sourceEnvelopeGateReason(file, requires);
	if (catalogEnvelopeReason)
		return `catalog candidate envelope: ${catalogEnvelopeReason}`;
...
function sourceEnvelopeGateReason(
	...
	return (
		excludedSourceDimensionsReason(file, envelope.excludeDimensions) ??
		...
		boundedSourceValueReason(
			"audio mean volume",
			file.audioMeanVolumeDb,
			envelope.minAudioMeanVolumeDb,
			undefined,
			"dB",
		)
	);
...
function excludedSourceDimensionsReason(
	file: SourceFileRecord,
	excluded: CandidateInputEnvelope["excludeDimensions"],
): string | undefined {
	if (
		excluded?.some(
	...
	return `dimensions ${file.width}x${file.height}px are explicitly excluded`;
}

function boundedSourceValueReason(
	field: "width" | "height" | "duration" | "audio mean volume",
...
function requiredMinDurationSec(scenario: Scenario): number {
	...
	if (scenario.op === "trim") {
		...
		return (
			Math.max(
				finite(range.startUs) ?? finite(options.startUs) ?? 0,
				finite(range.endUs) ?? finite(options.endUs) ?? 0,
			) / 1e6
		);
	}
	if (scenario.op === "seek")
		return Math.max(0, finite(options.tUs) ?? 0) / 1e6;
	return 0;
}

/** Full-range no-op invariants are meaningful only for candidates with that same full duration. */
function fixedDurationContractReason(
	scenario: Scenario,
	file: SourceFileRecord,
): string | undefined {
	const options = (scenario.options ?? {}) as Record<string, unknown>;
	if (
		scenario.op !== "trim" ||
		options.invariant !== TRIM_NOOP_IDENTITY_INVARIANT
	)
		return undefined;
	const range = isRecord(options.range) ? options.range : {};
	const endUs =
		typeof range.endUs === "number" && Number.isFinite(range.endUs)
			? range.endUs
...
function isRotatableCencMp4(row: ScenarioSourceRow): boolean {
	...
	return row.files.some(
		(file) =>
			file.keys?.scheme !== undefined && CENC_MP4_SCHEMES.has(file.keys.scheme),
	);
}

function isProtectedProbeCencScenario(scenario: Scenario): boolean {
	return (
		scenario.family === "probe" &&
		scenario.op === "probe" &&
		(scenario.requires.encryption ?? []).some((scheme) =>
			CENC_MP4_SCHEMES.has(scheme),
		)
	);
}
...
function candidateIdentity(
	...
	return canonicalSha256({
		schema: "selection-candidate-identity@1",
		scenarioId,
		inputs,
	});
}

function validateReplayCandidate(
	scenarioId: string,
	candidate: SelectionCandidateManifest,
): void {
	if (candidate.scenarioId !== scenarioId) {
		throw new SelectionPolicyError(
			...
			`recorded candidate belongs to '${candidate.scenarioId}'`,
		);
	}
	...
	if (
		...
		candidate.candidateIdentity !==
			candidateIdentity(scenarioId, candidate.inputs) ||
		...
		throw new SelectionPolicyError(
			...
			"recorded candidate identity is not internally coherent",
		);
	}
}

function bakedProvenance(
	logicalPath: string,
	asset: BakedAssetRecord,
	...
	return deepFreeze({
		...
		activity: {
			...
			...(file.provider
				...
				: file.cleartextBase?.provider
					? { provider: file.cleartextBase.provider }
					: {}),
		...
		...(file.cleartextBase
			? {
					derivation: {
						inputSha256: file.cleartextBase.sha256,
...
function compareRejections(
	a: CandidateRejection,
	b: CandidateRejection,
): number {
	return (
		(a.selectedFile ?? "").localeCompare(b.selectedFile ?? "") ||
		a.reasonCode.localeCompare(b.reasonCode) ||
		a.detail.localeCompare(b.detail)
	);
}

function compareOracleSets(
	a: readonly OracleId[],
	b: readonly OracleId[],
): number {
