...
export type WebCodecsConfigRecipe =
	| {
			role: "video-decoder";
			source: "selected-input-video";
			trackIndex?: number;
	  }
	| {
			role: "audio-decoder";
			source: "selected-input-audio";
			trackIndex?: number;
	  }
...
export function scenarioAssetIds(
	scenario: Pick<ScenarioSpec, "input">,
): string[] {
	return Array.isArray(scenario.input) ? scenario.input : [scenario.input];
}

export function groupScenariosByFeature(
	scenarios: Scenario[],
	...
	return SCENARIO_FAMILY_ORDER.filter((family) => byFamily.has(family)).map(
		(family) => ({
			id: family,
...
export interface CandidateInputEnvelope {
	...
	/** FFmpeg volumedetect mean-volume floor for source audio used by content-sensitive oracles. */
	minAudioMeanVolumeDb?: number;
	/** Video geometries that cannot discriminate a geometry-changing scenario from identity. */
	excludeDimensions?: readonly Readonly<{ width: number; height: number }>[];
}
...
export function registerScenarioMutationHandler(
	id: string,
	handler: ScenarioMutationHandler,
): void {
	if (!HANDLER_ID_PATTERN.test(id))
		throw new TypeError(
			`mutation handler id is invalid: ${JSON.stringify(id)}`,
		);
	if (mutationHandlers.has(id))
		throw new Error(`mutation handler already registered: ${id}`);
		...
		super(
			`${first.scenarioId} at ${first.path} [${first.code}]: ${first.message}` +
				(diagnostics.length > 1
					? ` (+${diagnostics.length - 1} more diagnostics)`
					: ""),
		);
...
export function validateScenarioDefinitionV2(
	...
	const scenarioId =
		typeof source?.id === "string" && source.id ? source.id : "<unknown>";
	...
	if (source.schemaVersion !== 2)
		add("schemaVersion", "SCHEMA_VERSION", "must equal 2");
	...
	if (!FAMILY_IDS.has(source.family as ScenarioFamily)) {
		add(
			"family",
			"SCENARIO_FAMILY",
			`unknown scenario family ${JSON.stringify(source.family)}`,
		);
	}
	...
	if (!OPERATION_IDS.has(source.op as Operation)) {
		add(
			"op",
			"SCENARIO_OPERATION",
			`unknown operation ${JSON.stringify(source.op)}`,
		);
	}
	...
	if (typeof source.notes !== "string")
		add("notes", "SCENARIO_NOTES", "must be a string");

	const authoredRenditionSet =
		isPlainRecord(source.options) && isPlainRecord(source.options.renditionSet)
			? source.options.renditionSet
			: undefined;
	if (
		authoredRenditionSet &&
		Array.isArray(source.renditionIds) &&
	...
	const operations = Array.isArray(requires?.operations)
		? requires.operations
		: [];
	if (typeof source.op === "string" && !operations.includes(source.op)) {
		add(
			"requires.operations",
			"OPERATION_REQUIREMENT_MISMATCH",
			`must include scenario op '${source.op}'`,
		);
	}
	const combinations = Array.isArray(requires?.anyOfCombinations)
		? requires.anyOfCombinations
		: [];
	if (
		typeof source.op === "string" &&
		!combinations.some(
			(entry) => isPlainRecord(entry) && entry.operation === source.op,
		)
	) {
		add(
			"requires.anyOfCombinations",
	...
	if (source.primaryMetric !== undefined) {
		if (!METRIC_IDS.has(source.primaryMetric as MetricId)) {
			add(
				...
				"must name a registered metric handler",
			);
		} else if (
			!Array.isArray(source.metrics) ||
			!source.metrics.includes(source.primaryMetric)
		) {
			add(
				"primaryMetric",
				"PRIMARY_METRIC_ABSENT",
				"must be present in metrics",
			);
		}
	}
	if (OPERATION_IDS.has(source.op as Operation)) {
		for (const [index, oracle] of arrayEntries(source.oracles)) {
			if (
				ORACLE_IDS.has(oracle as OracleId) &&
				!ORACLES_BY_OPERATION[source.op as Operation].has(oracle as OracleId)
			) {
				add(
					`oracles[${index}]`,
					"ORACLE_OPERATION_MISMATCH",
					`'${oracle}' is not applicable to '${source.op}'`,
				);
			}
		}
		for (const [index, metric] of arrayEntries(source.metrics)) {
			if (
				METRIC_IDS.has(metric as MetricId) &&
				!METRICS_BY_OPERATION[source.op as Operation].has(metric as MetricId)
			) {
				add(
					`metrics[${index}]`,
					"METRIC_OPERATION_MISMATCH",
					`'${metric}' is not applicable to '${source.op}'`,
				);
			}
		}
	}
...
export function scenarioDefinitionProjection(
	...
	return cloneDefinitionValue({
		...
		...(definition.candidateEnvelope
			? { candidateEnvelope: definition.candidateEnvelope }
			: {}),
		options: definition.options ?? {},
		requires: definition.requires,
		oracles: definition.oracles,
		metrics: definition.metrics,
		...(definition.primaryMetric !== undefined
...
export function validateScenarioBattery(
	...
	return scenarios.flatMap((scenario) =>
		validateScenarioDefinitionV2(
			scenarioDefinitionProjection(scenario),
			context,
		),
	);
}
...
export function matchRequirementCombination(
	...
	return requires.anyOfCombinations.find((combination) =>
		combinationMatches(combination, request),
	);
}
...
function normalizeScenarioDefinition(
	...
	const rawInputs =
		spec.inputs !== undefined
			? spec.inputs
			: (Array.isArray(spec.input) ? spec.input : [spec.input]).map(
					(assetId, index) => ({
						assetId,
	...
	const combinations = record.anyOfCombinations ?? [
		{
			...
			...((tokens.audioCodecsIn ?? tokens.audioCodecs)
				...
				: {}),
			...(operationHasEncodedOutput(operation) &&
			(tokens.videoCodecsOut ?? tokens.videoCodecs)
				? { videoCodecsOut: tokens.videoCodecsOut ?? tokens.videoCodecs }
				: {}),
			...(operationHasEncodedOutput(operation) &&
			(tokens.audioCodecsOut ?? tokens.audioCodecs)
...
function validateInputs(
	...
	for (const [index, entry] of value.entries()) {
		...
		if (
			...
			context.assetExists &&
			!context.assetExists(entry.assetId, scenarioId)
		) {
			add(
				`${path}.assetId`,
				"ASSET_NOT_FOUND",
				`asset '${entry.assetId}' does not resolve`,
			);
		}
		if (
			...
			add(
				`${path}.variantId`,
				"INPUT_VARIANT_ID",
				"must be a stable non-empty variant id",
			);
		} else if (variants.has(entry.variantId)) {
			add(
				`${path}.variantId`,
				"DUPLICATE_INPUT_VARIANT",
				`duplicate variant id '${entry.variantId}'`,
			);
...
function validateCandidateEnvelope(value: unknown, add: AddDiagnostic): void {
	...
	for (const key of [
		...
		if (
			bound !== undefined &&
			(!Number.isSafeInteger(bound) || (bound as number) <= 0)
		) {
			add(
				`candidateEnvelope.${key}`,
				"CANDIDATE_ENVELOPE_DIMENSION",
	...
	for (const key of ["minDurationSec", "maxDurationSec"] as const) {
		...
		if (
			...
			add(
				`candidateEnvelope.${key}`,
				"CANDIDATE_ENVELOPE_DURATION",
				"must be finite and non-negative",
			);
		}
	}
	const audioFloor = value.minAudioMeanVolumeDb;
	if (
		audioFloor !== undefined &&
		(typeof audioFloor !== "number" ||
			!Number.isFinite(audioFloor) ||
			audioFloor < -160 ||
			audioFloor > 24)
	) {
		add(
			"candidateEnvelope.minAudioMeanVolumeDb",
			"CANDIDATE_ENVELOPE_AUDIO_LEVEL",
			"must be finite and within [-160, 24] dB",
		);
	}
	validateCandidateExcludedDimensions(value.excludeDimensions, add);
	validateCandidateEnvelopeRange(value, "minWidth", "maxWidth", add);
	validateCandidateEnvelopeRange(value, "minHeight", "maxHeight", add);
	validateCandidateEnvelopeRange(
		value,
		"minDurationSec",
		"maxDurationSec",
		add,
...
function validateCandidateExcludedDimensions(
	value: unknown,
	add: AddDiagnostic,
): void {
	if (value === undefined) return;
	if (!Array.isArray(value) || value.length === 0) {
		add(
			"candidateEnvelope.excludeDimensions",
			"CANDIDATE_ENVELOPE_EXCLUDED_DIMENSIONS",
	...
	for (const [index, entry] of value.entries()) {
		const path = `candidateEnvelope.excludeDimensions[${index}]`;
		if (!isPlainRecord(entry)) {
			add(
				path,
				"CANDIDATE_ENVELOPE_EXCLUDED_DIMENSION",
				"must be a plain object",
			);
			continue;
		}
		rejectUnknownKeys(entry, CANDIDATE_DIMENSION_KEYS, path, add);
		let valid = true;
		for (const axis of ["width", "height"] as const) {
			if (!Number.isSafeInteger(entry[axis]) || (entry[axis] as number) <= 0) {
				add(
					`${path}.${axis}`,
					"CANDIDATE_ENVELOPE_EXCLUDED_DIMENSION",
					"must be a positive safe integer",
		...
		if (!valid) continue;
		const identity = `${entry.width}x${entry.height}`;
		if (seen.has(identity)) {
			add(
				path,
				"CANDIDATE_ENVELOPE_EXCLUDED_DIMENSION_DUPLICATE",
				"must be unique",
			);
...
function validateCandidateEnvelopeRange(
	...
	if (
		...
		add(
			`candidateEnvelope.${minKey}`,
			"CANDIDATE_ENVELOPE_RANGE",
			`must not exceed ${maxKey}`,
		);
	}
}
...
function validateOptions(
	...
	rejectUnknownKeys(
		value,
		OPTION_KEYS[operation as Operation],
		"options",
		add,
		"ILLEGAL_OPERATION_OPTION",
	);
	if (
		OUTPUT_CONTAINER_OPERATIONS.has(operation as Operation) &&
		(typeof value.container !== "string" || value.container.trim().length === 0)
	) {
		add(
			"options.container",
			"OUTPUT_CONTAINER_REQUIRED",
			`operation '${operation}' requires a container`,
		);
	}
	if (
		operation === "decodeFrames" &&
		(!Number.isSafeInteger(value.maxFrames) || (value.maxFrames as number) <= 0)
	) {
		add(
			"options.maxFrames",
			"DECODE_FRAME_LIMIT",
		...
		if (value.variants !== undefined) {
			...
			if (!Array.isArray(value.variants) || value.variants.length === 0) {
				add(
					"options.variants",
					"ABR_VARIANTS",
					"must be a non-empty array of video rungs",
				);
			} else {
				for (const [index, variant] of value.variants.entries()) {
					validateTranscodeVideoOptions(
						variant,
	...
	if (operation === "transcode" && value.renditionSet !== undefined) {
		if (!isPlainRecord(value.renditionSet)) {
			...
			if (
				typeof renditionSet.id !== "string" ||
				renditionSet.id.trim().length === 0
			) {
				add(
					"options.renditionSet.id",
					"RENDITION_SET_ID",
					"must be a non-empty string",
				);
			}
			validateIdArray(
				renditionSet.renditionIds,
				"options.renditionSet.renditionIds",
				undefined,
				false,
				add,
			...
			if (
				!Array.isArray(renditionSet.switchPointsUs) ||
				renditionSet.switchPointsUs.length === 0
			) {
				add(
					"options.renditionSet.switchPointsUs",
					"RENDITION_SET_SWITCH_POINTS",
					"must be a non-empty array",
				);
			} else {
				for (const [index, point] of renditionSet.switchPointsUs.entries()) {
					if (
						...
						(index > 0 &&
							(point as number) <=
								(renditionSet.switchPointsUs[index - 1] as number))
					) {
						add(
			...
			if (
				renditionSet.segmentMode !== "random-access" &&
				renditionSet.segmentMode !== "segments"
			) {
				add(
					"options.renditionSet.segmentMode",
					"RENDITION_SET_SEGMENT_MODE",
					"must be 'random-access' or 'segments'",
				);
			}
		}
	}
	if (operation === "decrypt") {
		if (typeof value.scheme !== "string")
			add("options.scheme", "DECRYPT_SCHEME", "must be a scheme string");
		if (!isPlainRecord(value.key))
			add("options.key", "DECRYPT_KEY", "must be a key descriptor object");
	}
}
...
function validateTranscodeVideoOptions(
	...
	if (!isPlainRecord(value)) {
		add(
			path,
			abrRung ? "ABR_RUNG_TYPE" : "TRANSCODE_VIDEO_TYPE",
			"must be a plain JSON object",
		);
		return;
	}
	...
	if (
		...
		(typeof value.codec !== "string" || value.codec.trim().length === 0)
	) {
		add(
			`${path}.codec`,
			"TRANSCODE_VIDEO_CODEC",
			"must be a non-empty codec string",
		);
	}
	if (abrRung && value.codec !== undefined && value.codec !== "h264") {
		add(
			`${path}.codec`,
			"ABR_RUNG_CODEC",
			"the H.264 ABR operation requires codec 'h264'",
		);
	}
	...
	for (const key of ["width", "height"] as const) {
		...
		if (abrRung && dimension === undefined) {
			add(
				`${path}.${key}`,
				"ABR_RUNG_DIMENSION_REQUIRED",
				"is required for every ABR rung",
			);
		} else if (
			dimension !== undefined &&
			(!Number.isSafeInteger(dimension) ||
				((abrRung || constrained) && (dimension as number) <= 0))
		) {
			add(
				`${path}.${key}`,
				"TRANSCODE_VIDEO_DIMENSION",
				"must be a positive safe integer",
			);
		}
	}
	...
	if (
		hasMaximum &&
		(!Number.isSafeInteger(value.maxAverageBitrate) ||
			(value.maxAverageBitrate as number) <= 0)
	) {
		add(
			`${path}.maxAverageBitrate`,
			"TRANSCODE_VIDEO_MAXIMUM_BITRATE",
			"must be a positive safe integer",
		);
	}
	...
	if ((hasMaximum || hasQuality) && value.bitrate === undefined) {
		add(
			`${path}.bitrate`,
			"TRANSCODE_QUALITY_BITRATE_REQUIRED",
			"is required by the quality/rate tuple",
		);
	}
	if (
	...
	if (hasQuality)
		validateTranscodeQuality(value.quality, `${path}.quality`, add);
}

function validateTranscodeQuality(
	value: unknown,
	path: string,
	add: AddDiagnostic,
): void {
	...
	rejectUnknownKeys(
		value,
		TRANSCODE_QUALITY_KEYS,
		path,
		add,
		"TRANSCODE_QUALITY_OPTION",
	);
	if (value.metric !== "ssim-luma-v1") {
		add(
			`${path}.metric`,
			"TRANSCODE_QUALITY_METRIC",
			"must equal 'ssim-luma-v1'",
		);
	}
	if (
	...
	if (
		...
		(!Number.isSafeInteger(value.samples) ||
			...
			(value.samples as number) > 256)
	) {
		add(
			`${path}.samples`,
			"TRANSCODE_QUALITY_SAMPLES",
			"must be a safe integer in [1, 256]",
		);
	}
}
...
function validateRequires(
	...
	if (!isPlainRecord(value.allOfTokens)) {
		...
		for (const [key, vocabulary] of REQUIREMENT_VOCABULARIES) {
			...
			if (
				...
				Array.isArray(value.allOfTokens[key]) &&
	...
	if (
		!Array.isArray(value.anyOfCombinations) ||
		value.anyOfCombinations.length === 0
	) {
		add(
			"requires.anyOfCombinations",
	...
	value.anyOfCombinations.forEach((entry, index) =>
		validateCombination(entry, index, operation, add),
	);
}
...
function validateCombination(
	...
	if (entry.browserConfigRecipes !== undefined) {
		if (!Array.isArray(entry.browserConfigRecipes)) {
			add(
				`${path}.browserConfigRecipes`,
				"BROWSER_CONFIG_RECIPES",
				"must be an array",
			);
		} else {
			entry.browserConfigRecipes.forEach((recipe, recipeIndex) => {
				...
				if (!BROWSER_ROLES.has(recipe.role as BrowserCodecRole)) {
					add(
						`${recipePath}.role`,
						"BROWSER_ROLE",
						"must name a WebCodecs role",
					);
				}
				if (
					!RECIPE_SOURCES.has(recipe.source as WebCodecsConfigRecipe["source"])
				) {
					add(
						`${recipePath}.source`,
						"BROWSER_CONFIG_SOURCE",
						"must name a supported config source",
					);
				}
				if (
					recipe.trackIndex !== undefined &&
					(!Number.isSafeInteger(recipe.trackIndex) ||
						(recipe.trackIndex as number) < 0)
				) {
					add(
						`${recipePath}.trackIndex`,
						"BROWSER_CONFIG_TRACK",
						"must be a non-negative safe integer",
					);
				}
				if (
					Array.isArray(entry.browserRoles) &&
					!entry.browserRoles.includes(recipe.role)
				) {
					add(
						`${recipePath}.role`,
						"BROWSER_ROLE_UNDECLARED",
						"must also appear in browserRoles",
					);
				}
			});
		}
	...
	for (const key of TOLERANCE_KEYS) {
		...
		if (entry < 0 || (key === "ssimMin" && entry > 1)) {
			add(
				`tolerances.${key}`,
				"TOLERANCE_RANGE",
				key === "ssimMin" ? "must be within [0, 1]" : "must be non-negative",
			);
		}
	}
}
...
function validateMutation(
	...
	if (!isPlainRecord(source.mutation)) {
		add(
			"mutation",
			"SCENARIO_MUTATION",
			"must be a declarative mutation object",
		);
		return;
	}
	...
	if (typeof id !== "string" || !HANDLER_ID_PATTERN.test(id)) {
		...
		context.mutationHandlerExists &&
		!context.mutationHandlerExists(id)
	) {
		add(
			"mutation.mutationId",
			"MUTATION_HANDLER_UNKNOWN",
			`unknown mutation handler '${id}'`,
		);
	}
	if (!isPlainRecord(source.mutation.parameters)) {
		...
		validateJsonValue(source.mutation.parameters, "mutation.parameters", add);
	const negativePath =
		source.family === "robustness" ||
		(typeof source.id === "string" &&
			NEGATIVE_SCENARIO_PATTERN.test(source.id)) ||
		(Array.isArray(source.oracles) &&
			source.oracles.includes("graceful-failure"));
	if (!negativePath) {
		add(
			"mutation",
			"MUTATION_SCOPE",
			"mutations are confined to negative/robustness scenarios",
		);
	}
}
...
function validateIdArray(
	...
	for (const [index, entry] of value.entries()) {
		if (typeof entry !== "string" || entry.trim().length === 0) {
			...
			add(
				`${path}[${index}]`,
				"SCENARIO_HANDLER_UNKNOWN",
				`unknown handler/token '${entry}'`,
			);
...
function validateJsonValue(
	...
	if (typeof value === "number") {
		if (!Number.isFinite(value))
			add(path, "JSON_NUMBER_FINITE", "must be a finite JSON number");
		return;
	}
	...
	if (Array.isArray(value)) {
		value.forEach((entry, index) =>
			validateJsonValue(entry, `${path}[${index}]`, add, active),
		);
	} else {
		for (const [key, entry] of Object.entries(value))
			validateJsonValue(entry, `${path}.${key}`, add, active);
	}
	active.delete(value);
}
...
function combinationMatches(
	...
	const inputVideo = request.inputs.flatMap((input) =>
		input.tracks
			.filter((track) => track.type === "video")
			.map((track) => track.codec),
	);
	const inputAudio = request.inputs.flatMap((input) =>
		input.tracks
	...
	if (
		combination.containersOut &&
		(!request.output ||
			!combination.containersOut.includes(request.output.container))
	)
		return false;
	if (
		combination.videoCodecsOut &&
		(!request.output?.videoCodec ||
			!combination.videoCodecsOut.includes(request.output.videoCodec))
	)
		return false;
	if (
		combination.audioCodecsOut &&
		(!request.output?.audioCodec ||
			!combination.audioCodecsOut.includes(request.output.audioCodec))
	)
		return false;
	if (
		combination.encryption &&
		(!request.encryption ||
			!combination.encryption.includes(request.encryption))
	)
		return false;
	if (
		combination.optionConstraints &&
		!recordContains(request.options, combination.optionConstraints)
	)
		return false;
	return true;
}
...
function deriveWebCodecsConfig(
	...
	const selected = selectTrack(
		request,
		recipe.role.startsWith("video") ? "video" : "audio",
		recipe.trackIndex,
	);
	switch (recipe.role) {
		case "video-decoder": {
			if (!selected || selected.track.type !== "video")
				throw configDerivationError(
					recipe,
					"selected video input metadata is absent",
				);
			const width = positiveNumber(selected.track.width, "codedWidth", recipe);
			const height = positiveNumber(
				selected.track.height,
				"codedHeight",
				recipe,
			);
			return {
				...
				config: {
					codec: webCodecsCodec(
						selected.track.nativeCodecTag ?? selected.track.codec,
					),
					codedWidth: width,
					codedHeight: height,
				},
		...
		case "audio-decoder": {
			if (!selected || selected.track.type !== "audio")
				throw configDerivationError(
					recipe,
					"selected audio input metadata is absent",
				);
			return {
				role: recipe.role,
				trackIndex: selected.index,
				config: {
					...
					sampleRate: positiveNumber(
						selected.track.sampleRate,
						"sampleRate",
						recipe,
					),
					numberOfChannels: positiveNumber(
						selected.track.channels,
						"numberOfChannels",
						recipe,
					),
				},
		...
		case "video-encoder": {
			const inputVideo =
				selected?.track.type === "video" ? selected.track : undefined;
			const width = positiveNumber(
				request.output?.width ?? inputVideo?.width,
				"width",
				recipe,
			);
			const height = positiveNumber(
				request.output?.height ?? inputVideo?.height,
				"height",
				recipe,
			);
			if (!request.output?.videoCodec)
				throw configDerivationError(recipe, "output video codec is absent");
			const videoOptions = isPlainRecord(request.options.video)
				? request.options.video
				: undefined;
			return {
				role: recipe.role,
				...(selected ? { trackIndex: selected.index } : {}),
				config: {
					codec: webCodecsCodec(request.output.videoCodec),
					width,
					height,
					...(request.output.frameRate
						? { framerate: request.output.frameRate }
						: {}),
					...(typeof videoOptions?.bitrate === "number"
		...
		case "audio-encoder": {
			const inputAudio =
				selected?.track.type === "audio" ? selected.track : undefined;
			if (!request.output?.audioCodec)
				throw configDerivationError(recipe, "output audio codec is absent");
			const audioOptions = isPlainRecord(request.options.audio)
			...
			return {
				...
				config: {
					...
					sampleRate: positiveNumber(
						request.output.sampleRate ?? inputAudio?.sampleRate,
						"sampleRate",
						recipe,
					),
					numberOfChannels: positiveNumber(
						request.output.channels ?? inputAudio?.channels,
						"numberOfChannels",
						recipe,
					),
					...(typeof audioOptions?.bitrate === "number"
...
function selectTrack(
	...
	| {
			track: ConcreteOperationRequest["inputs"][number]["tracks"][number];
			index: number;
	  }
	| undefined {
	const tracks = request.inputs
...
function configDerivationError(
	recipe: WebCodecsConfigRecipe,
	detail: string,
): TypeError {
	return new TypeError(
		`cannot derive ${recipe.role} config from ${recipe.source}: ${detail}`,
	);
}
...
function positiveNumber(
	...
	if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
		throw configDerivationError(
			recipe,
			`${field} is not a positive finite number`,
		);
	}
	return value;
}
...
function deriveRenditionIds(options: JsonObject): string[] {
	...
	return variants.map((variant, index) =>
		...
		typeof variant.renditionId === "string" &&
		variant.renditionId
			? variant.renditionId
			: `rendition-${index + 1}`,
	);
...
function cloneDefinitionValue<T>(
	value: T,
	...
	seen.set(value, out);
	for (const [key, entry] of Object.entries(value))
		out[key] = cloneDefinitionValue(entry, seen);
	return out as T;
}
...
const NEGATIVE_SCENARIO_PATTERN =
	/\/(?:negative|malformed|corrupt|fuzz|truncat|destroy|invalid|empty|graceful)/i;
const FAMILY_IDS = new Set<ScenarioFamily>(
	SCENARIO_FAMILY_MANIFEST.map((entry) => entry.family),
);
...
const CANDIDATE_ENVELOPE_KEYS = new Set<keyof CandidateInputEnvelope>([
	...
	"minDurationSec",
	"maxDurationSec",
	"minAudioMeanVolumeDb",
	"excludeDimensions",
]);
const CANDIDATE_DIMENSION_KEYS = new Set(["width", "height"]);
...
const RENDITION_SET_KEYS = new Set([
	"id",
	"renditionIds",
	"switchPointsUs",
	"segmentMode",
...
const REQUIREMENT_VOCABULARIES = [
	...
	["containersIn", CONTAINERS],
	["containersOut", CONTAINERS],
	["videoCodecs", VIDEO_CODECS],
	["audioCodecs", AUDIO_CODECS],
	["videoCodecsIn", VIDEO_CODECS],
	["audioCodecsIn", AUDIO_CODECS],
	["videoCodecsOut", VIDEO_CODECS],
	["audioCodecsOut", AUDIO_CODECS],
	["encryption", ENCRYPTION_SCHEMES],
	["features", FEATURES],
] as const;
...
const OUTPUT_CONTAINER_OPERATIONS = new Set<Operation>([
	"remux",
	"transcode",
	"trim",
	"mux",
...
export type OracleReductionStatus =
	| OracleVerdict
 ...
 */
export function reduceOracleOutcomes(
	outcomes: readonly OracleOutcome[],
): OracleReduction {
	const sorted = [...outcomes].sort((a, b) =>
		oracleOutcomeSortKey(a).localeCompare(oracleOutcomeSortKey(b)),
	);
	const verdict = (value: OracleVerdict): OracleOutcome | undefined =>
		sorted.find(
			(outcome) => outcome.state === "VERDICT" && outcome.verdict === value,
		);
	...
	const browser = sorted.find(
		(outcome) =>
			outcome.state === "UNAVAILABLE" && outcome.status === "NA_BROWSER",
	);
	if (browser)
		return reduced("NA_BROWSER", "ORACLE_ALL_UNAVAILABLE_BROWSER", browser);
	const asset = sorted.find(
		(outcome) =>
			outcome.state === "UNAVAILABLE" && outcome.status === "NA_ASSET",
	);
...
function oracleOutcomeDetail(outcome: OracleOutcome): string {
	...
	return (
		`${outcome.state === "ERROR" ? "errored" : `unavailable (${outcome.status})`} ` +
...
export function reduceExhaustiveStatuses(
	...
	const count = (status: ResultStatus): number =>
	...
	const terminalIntrinsicCoverage =
		...
		counts.naAsset === 0 &&
		counts.skipped === 0;
	const grade: ExhaustiveCoverage["grade"] = terminalIntrinsicCoverage
		? "full"
		: valid > 0
			? "partial"
			: "none";
