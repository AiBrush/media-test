...
export const SELECTION_ALGORITHM_ID =
	"candidate-identity-lexicographic-min-v1" as const;
export const SELECTION_MANIFEST_SCHEMA_VERSION =
	"media-selection-manifest@1" as const;
...
export interface SourceFileRecord {
	...
	width?: number | null;
	height?: number | null;
	/** Source-bound FFmpeg volumedetect mean volume; null means measured silence/unavailable signal. */
...
export type BakedManifestValidationResult =
	| {
			state: "VALID";
			manifest: ValidatedBakedCorpusManifest;
			issues: readonly CatalogIssue[];
	  }
...
const METAMORPHIC_KEYS = new Set([
	"oracle",
	"invariant",
	"cleartextBaseSha256",
]);
...
export function canonicalJson(value: unknown): string {
	...
	const encode = (item: unknown, inArray: boolean): string | undefined => {
		...
		if (item === undefined) {
			if (inArray)
				throw new TypeError("canonicalJson rejects undefined array entries");
			return undefined;
		}
		if (typeof item !== "object")
			throw new TypeError(`canonicalJson rejects ${typeof item}`);
		if (active.has(item))
		...
		try {
			...
			if (prototype !== Object.prototype && prototype !== null) {
				throw new TypeError(
					"canonicalJson accepts only arrays and plain objects",
				);
			}
	...
	if (encoded === undefined)
		throw new TypeError("canonicalJson root cannot be undefined");
	return encoded;
}
...
export function parseScenarioSourceCatalog(
	text: string,
	...
	for (const [index, line] of text.split(/\r?\n/u).entries()) {
		...
		try {
			...
			issues.push(
				issue(
					"CATALOG_JSON_PARSE",
					`line ${index + 1}: ${errorMessage(error)}`,
					{ line: index + 1 },
				),
			);
			continue;
		...
		if (previous !== undefined) {
			issues.push(
				issue(
					...
					`duplicate scenarioId '${row.scenarioId}'`,
					{
						line: previous,
...
export function parseBakedCorpusManifest(
	value: unknown,
): BakedManifestValidationResult {
	const issues: CatalogIssue[] = [];
	if (!isRecord(value)) {
		return deepFreeze({
			state: "INVALID",
			issues: [
				issue("BAKED_MANIFEST_NOT_OBJECT", "baked manifest must be an object"),
			],
		});
	}
	rejectUnknownKeys(
		value,
		new Set(["$schema", "suiteCorpusVersion", "assets"]),
		"baked manifest",
		issues,
	);
	const corpusVersion = stringField(
		value,
		"suiteCorpusVersion",
		"baked manifest",
		issues,
	...
	if (!Array.isArray(rawAssets))
		issues.push(
			issue("BAKED_ASSETS_NOT_ARRAY", "baked manifest assets must be an array"),
		);
	const assets: BakedAssetRecord[] = [];
	const ids = new Set<string>();
	for (const [index, raw] of (Array.isArray(rawAssets)
		? rawAssets
		: []
	).entries()) {
		...
		if (!isRecord(raw)) {
			issues.push(
				issue("BAKED_ASSET_NOT_OBJECT", `${path} must be an object`, { path }),
			);
			continue;
		}
		rejectUnknownKeys(
		...
		if (ids.has(id))
			issues.push(
				issue("BAKED_DUPLICATE_ASSET_ID", `duplicate baked asset id '${id}'`, {
					path: id,
				}),
			);
		ids.add(id);
		assets.push({
			...
			...(raw.expectedSizeBytes === undefined
				? {}
				: {
						expectedSizeBytes: optionalSafeSize(
							raw.expectedSizeBytes,
							`${path}.expectedSizeBytes`,
							issues,
						),
					}),
			...(raw.codecs === undefined
				? {}
				: { codecs: stringArray(raw.codecs, `${path}.codecs`, issues) }),
		});
	}
...
export function scenarioSourceMap(
	catalog: ValidatedScenarioSourceCatalog,
): ReadonlyMap<string, ScenarioSourceRow> {
...
export const CORPUS_DELIVERY_INTEGRITY_ERROR_KIND =
	"media-test/corpus-delivery-integrity-error@1" as const;
...
export function isCorpusDeliveryIntegrityError(
	value: unknown,
): value is CorpusDeliveryIntegrityError {
	if (!value || typeof value !== "object") return false;
	const item = value as Record<string, unknown>;
	return (
		item.kind === CORPUS_DELIVERY_INTEGRITY_ERROR_KIND &&
...
export async function verifyContentStream(
	...
	if (!Number.isSafeInteger(chunkSizeBytes) || chunkSizeBytes <= 0) {
		throw new RangeError(
			"verifyContentStream chunkSizeBytes must be a positive safe integer",
		);
	}
	...
	try {
		...
		return deepFreeze({
			...
			issue: corpusIssue(
				"CORPUS_FETCH_FAILED",
				`failed to fetch '${identity.logicalPath}': ${errorMessage(error)}`,
				identity,
			),
		});
	}
	...
	try {
		while (true) {
			...
			try {
				...
				if (!Number.isSafeInteger(actualSizeBytes))
					throw new RangeError("content stream exceeds safe byte length");
				let offset = 0;
		...
		return deepFreeze({
			...
			issue: corpusIssue(
				"CORPUS_FETCH_FAILED",
				`failed to stream '${identity.logicalPath}': ${errorMessage(error)}`,
				identity,
			),
		});
	} finally {
...
export async function verifyContentSet(
	...
	const verified = results.filter(
		(result): result is VerifiedContent => result.state === "VERIFIED",
	);
	...
	return deepFreeze({
		verified,
		issues: [...issueByIdentity.values()],
		eligible: verified.length,
	});
}
...
export function contentIdentityDigest(
...
export function canonicalSetDigest(
	identities: readonly ContentIdentity[],
): string {
	return canonicalSha256({
...
function validateSourceRow(
	...
	line: number,
	issues: CatalogIssue[],
): ScenarioSourceRow | undefined {
	const context = `catalog line ${line}`;
	if (!isRecord(raw)) {
		issues.push(
			issue("CATALOG_ROW_NOT_OBJECT", `${context} must be an object`, { line }),
		);
		return undefined;
	}
	rejectUnknownKeys(raw, SOURCE_ROW_KEYS, context, issues, { line });
	const scenarioId = normalizedPathField(raw, "scenarioId", context, issues, {
		line,
	});
	const classValue = raw.class;
	if (
		typeof classValue !== "string" ||
		!SOURCE_CLASSES.has(classValue as SourceClass)
	) {
		issues.push(
			issue(
				"CATALOG_CLASS_INVALID",
				`${context}.class is not a known source class`,
				{ line, scenarioId },
			),
		);
	}
	const requires = validateRequires(
		raw.requires,
		context,
		issues,
		line,
	...
	if (!Array.isArray(raw.files)) {
		issues.push(
			issue("CATALOG_FILES_NOT_ARRAY", `${context}.files must be an array`, {
				line,
				scenarioId,
			}),
		);
	...
	for (const [index, file] of (Array.isArray(raw.files)
		? raw.files
		: []
	).entries()) {
	...
	if (
		(classValue === "REAL" || classValue === "DERIVED") &&
		files.length === 0
	) {
		issues.push(
			issue(
				"CATALOG_CLASS_REQUIRES_FILES",
				`${classValue} row '${scenarioId ?? "?"}' must contain files`,
				{
					line,
					scenarioId,
				},
	...
	const reason = optionalString(raw.reason, `${context}.reason`, issues, {
		line,
		scenarioId,
	});
	const note = optionalString(raw.note, `${context}.note`, issues, {
		line,
		scenarioId,
	});
	if (
	...
	for (const file of files)
		validateFileCoherence(file, requires, context, issues, line, scenarioId);
	return {
		scenarioId,
		requires,
...
function validateRequires(
	...
	if (!isRecord(raw)) {
		issues.push(
			issue(
				"CATALOG_REQUIRES_NOT_OBJECT",
				`${context}.requires must be an object`,
				{ line, scenarioId },
			),
		);
		return undefined;
	}
	rejectUnknownKeys(raw, REQUIRES_KEYS, `${context}.requires`, issues, {
		line,
		scenarioId,
	});
	const container = normalizedTokenField(
		raw,
		"container",
		`${context}.requires`,
		issues,
	...
	if (typeof video !== "boolean") {
		issues.push(
			issue(
				"CATALOG_VIDEO_FLAG_INVALID",
				`${context}.requires.video must be boolean`,
				{ line, scenarioId },
			),
		);
	}
	const videoCodecs = normalizedTokenArray(
		raw.videoCodecs,
		`${context}.requires.videoCodecs`,
		issues,
		{ line, scenarioId },
	);
	const audioCodecs = normalizedTokenArray(
		raw.audioCodecs,
		`${context}.requires.audioCodecs`,
		issues,
		{ line, scenarioId },
	);
	const encryption =
		raw.encryption === undefined || raw.encryption === null
			? raw.encryption
			: normalizedTokenArray(
					raw.encryption,
					`${context}.requires.encryption`,
					issues,
					{ line, scenarioId },
				);
	const location = { line, scenarioId };
	const minWidth = optionalSourceRequirementBound(
		raw.minWidth,
		`${context}.requires.minWidth`,
		"dimension",
		issues,
		location,
	);
	const maxWidth = optionalSourceRequirementBound(
		raw.maxWidth,
		`${context}.requires.maxWidth`,
		"dimension",
		issues,
		location,
	);
	const minHeight = optionalSourceRequirementBound(
		raw.minHeight,
		`${context}.requires.minHeight`,
		"dimension",
		issues,
		location,
	);
	const maxHeight = optionalSourceRequirementBound(
		raw.maxHeight,
		`${context}.requires.maxHeight`,
		"dimension",
		issues,
		location,
	);
	...
	validateSourceRequirementRange(
		minWidth,
		maxWidth,
		"width",
		`${context}.requires`,
	...
	if (!container || typeof video !== "boolean" || !videoCodecs || !audioCodecs)
		return undefined;
	if (video && videoCodecs.length === 0) {
		issues.push(
			issue(
				"CATALOG_VIDEO_CODEC_REQUIRED",
				`${context}.requires.video=true needs a video codec`,
				{ line, scenarioId },
			),
		);
	}
...
function validateSourceFile(
	...
	if (!isRecord(raw)) {
		issues.push(
			issue("CATALOG_FILE_NOT_OBJECT", `${context} must be an object`, {
				line,
				scenarioId,
			}),
		);
	...
	rejectUnknownKeys(raw, SOURCE_FILE_KEYS, context, issues, {
		line,
		scenarioId,
	});
	const file = normalizedPathField(raw, "file", context, issues, {
		line,
		scenarioId,
	});
	const container = normalizedTokenField(raw, "container", context, issues, {
		line,
		scenarioId,
	});
	const videoCodecs = normalizedTokenArray(
	...
	const sha256 = shaField(raw, "sha256", context, issues, {
		line,
		scenarioId,
		path: file,
	});
	const sizeBytes = safeSizeField(raw, "sizeBytes", context, issues, {
		line,
		scenarioId,
		path: file,
	});
	...
	if (
		typeof output.audioMeanVolumeDb === "number" &&
		(output.audioMeanVolumeDb < -160 || output.audioMeanVolumeDb > 24)
	) {
		issues.push(
			issue(
				"CATALOG_AUDIO_LEVEL_INVALID",
				`${context}.audioMeanVolumeDb must be within [-160, 24] dB`,
				{ line, scenarioId, path: file },
			),
		);
	}
	...
	if (
		output.poolPath !== undefined &&
		normalizeRelativePath(output.poolPath) !== output.poolPath
	) {
		issues.push(
			issue(
				"CATALOG_PATH_NOT_NORMALIZED",
				`${context}.poolPath is not a normalized relative path`,
				{
					line,
					scenarioId,
	...
	if (raw.hlsFiles !== undefined) {
		const hlsFiles = pathArray(raw.hlsFiles, `${context}.hlsFiles`, issues, {
			line,
			scenarioId,
			path: file,
		});
	...
	if (raw.keys !== undefined) {
		const keys = validateKeys(raw.keys, `${context}.keys`, issues, {
			line,
			scenarioId,
			path: file,
		});
	...
	if (raw.cleartextBase !== undefined) {
		const base = validateCleartextBase(
			raw.cleartextBase,
			`${context}.cleartextBase`,
			issues,
			{ line, scenarioId, path: file },
		);
...
function validateKeys(
	...
	const ivHex = optionalLowerHex(
		raw.ivHex,
		`${context}.ivHex`,
		issues,
		location,
	);
	const keyUri = optionalString(
		raw.keyUri,
		`${context}.keyUri`,
		issues,
		location,
	);
	const ivMode = optionalString(
		raw.ivMode,
		`${context}.ivMode`,
		issues,
		location,
...
function validateCleartextBase(
	...
	const poolPath = normalizedPathField(
		raw,
		"poolPath",
		context,
		issues,
	...
	Object.assign(
		...
		optionalStringProps(
			raw,
			["provider", "sourcePageUrl", "downloadUrl"],
			context,
			issues,
			location,
		),
...
function validateEvidence(
	...
	const sufficientOracleSets = oracleSets(
		raw.sufficientOracleSets,
		`${context}.sufficientOracleSets`,
		issues,
		location,
	);
	const requiredOracles =
		raw.requiredOracles === undefined
			? undefined
			: enumArray(
					raw.requiredOracles,
					ORACLE_IDS,
					`${context}.requiredOracles`,
					issues,
					location,
				);
	...
	if (raw.metamorphicSurvivor !== undefined) {
		if (!isRecord(raw.metamorphicSurvivor)) {
			issues.push(
				issue(
					"CATALOG_METAMORPHIC_NOT_OBJECT",
					`${context}.metamorphicSurvivor must be an object`,
					location,
				),
			);
		} else {
			...
			if (oracle !== "property-invariant") {
				issues.push(
					issue(
						"CATALOG_METAMORPHIC_ORACLE_INVALID",
						`${context}.metamorphicSurvivor.oracle must be property-invariant`,
						location,
					),
				);
...
function validateContract(
	...
	const scenarioId = normalizedPathField(
		raw,
		"scenarioId",
		context,
		issues,
	...
	const scenarioContractDigest = shaField(
		raw,
		"scenarioContractDigest",
		context,
		issues,
	...
	return {
		...
		scenarioContractDigest,
		sourceSha256,
		...(kind === "same-contract" || kind === "robustness-variant"
			? { kind }
			: {}),
	};
}
...
function validateFileCoherence(
	...
	if (file.container !== requires.container) {
		issues.push(
			warning(
				"CATALOG_CONTAINER_INCOHERENT",
				`${context}: ${file.file} container '${file.container}' does not match '${requires.container}'`,
				location,
			),
		);
	}
	for (const codec of requires.videoCodecs) {
		if (!file.videoCodecs.includes(codec))
			issues.push(
				warning(
					"CATALOG_VIDEO_CODEC_INCOHERENT",
					`${file.file} lacks '${codec}'`,
					location,
				),
			);
	}
	for (const codec of requires.audioCodecs) {
		if (!file.audioCodecs.includes(codec))
			issues.push(
				warning(
					"CATALOG_AUDIO_CODEC_INCOHERENT",
					`${file.file} lacks '${codec}'`,
					location,
				),
			);
	...
	if (
		...
		issues.push(
			issue(
				"CATALOG_AUDIO_ONLY_HAS_VIDEO",
				`${file.file} carries video for an audio-only row`,
				location,
			),
		);
	}
	if (file.evidence && file.evidence.sourceSha256 !== file.sha256) {
		issues.push(
			issue(
				"CATALOG_EVIDENCE_DIGEST_MISMATCH",
				`${file.file} evidence covers a different source digest`,
				location,
			),
		);
	}
	if (file.contract) {
		if (file.contract.sourceSha256 !== file.sha256) {
			issues.push(
				issue(
					"CATALOG_CONTRACT_DIGEST_MISMATCH",
					`${file.file} contract covers a different source digest`,
					location,
				),
			);
		}
		if (file.contract.scenarioId !== scenarioId) {
			issues.push(
				issue(
					"CATALOG_CONTRACT_SCENARIO_MISMATCH",
					`${file.file} contract names '${file.contract.scenarioId}'`,
					location,
				),
			);
		}
...
function diagnoseCandidateDuplicates(
		...
		if (paths.has(file.file)) {
			issues.push(
				issue(
					...
					`duplicate path '${file.file}' in '${row.scenarioId}'`,
					{
						scenarioId: row.scenarioId,
						path: file.file,
					},
		...
		if (digests.has(file.sha256)) {
			issues.push(
				issue(
					...
					`duplicate content digest '${file.sha256}' in '${row.scenarioId}'`,
					{
						scenarioId: row.scenarioId,
						path: file.file,
					},
...
function normalizeSourceRow(row: ScenarioSourceRow): ScenarioSourceRow {
	return {
		...
		requires: {
			...
			...(Array.isArray(row.requires.encryption)
				? { encryption: [...row.requires.encryption].sort() }
				: {}),
		},
		files: row.files
			.map((file) => ({
			...
			.sort(
				(a, b) =>
					a.file.localeCompare(b.file) || a.sha256.localeCompare(b.sha256),
			),
	};
}
...
function stringField(
	...
	if (
		...
		issues.push(
			issue(
				"CATALOG_STRING_INVALID",
				`${context}.${key} must be a nonempty trimmed string`,
				location,
			),
		);
		return undefined;
...
function optionalString(
	...
	if (value === null) {
		issues.push(
			warning(
				"CATALOG_OPTIONAL_STRING_OMITTED",
				`${context} is null and was omitted`,
				location,
			),
		);
		return undefined;
	...
	if (typeof value !== "string") {
		issues.push(
			issue("CATALOG_STRING_INVALID", `${context} must be a string`, location),
		);
		return undefined;
	}
	if (value.length === 0) {
		issues.push(
			warning(
				"CATALOG_OPTIONAL_STRING_OMITTED",
				`${context} is empty and was omitted`,
				location,
			),
		);
		return undefined;
	...
	if (value.trim().length === 0 || value !== value.trim()) {
		issues.push(
			issue(
				"CATALOG_STRING_INVALID",
				`${context} must be a normalized trimmed string`,
				location,
			),
		);
		return undefined;
...
function normalizedTokenField(
	...
	if (field !== normalized || !/^[a-z0-9][a-z0-9._+-]*$/u.test(field)) {
		issues.push(
			issue(
				"CATALOG_TOKEN_NOT_NORMALIZED",
				`${context}.${key} must be a normalized lowercase token`,
				location,
			),
		);
		return undefined;
	...
	for (const [index, value] of raw.entries()) {
		if (
			...
			issues.push(
				issue(
					"CATALOG_TOKEN_NOT_NORMALIZED",
					`${context}[${index}] is not a normalized lowercase token`,
					location,
				),
			);
			continue;
		...
		if (seen.has(value))
			issues.push(
				issue(
					"CATALOG_DUPLICATE_TOKEN",
					`${context} repeats '${value}'`,
					location,
				),
			);
		else {
...
function normalizedPathField(
	...
	if (normalizeRelativePath(field) !== field) {
		issues.push(
			issue(
				"CATALOG_PATH_NOT_NORMALIZED",
				`${context}.${key} is not a normalized relative path`,
				{
					...location,
					path: field,
				},
	...
	if (typeof field !== "string" || !LOWER_SHA256.test(field)) {
		issues.push(
			issue(
				...
				`${context}.${key} must be lowercase 64-hex SHA-256`,
				location,
			),
		);
		return undefined;
...
function lowerHexField(
	...
	if (
		typeof field !== "string" ||
		field.length % 2 !== 0 ||
		!LOWER_HEX.test(field)
	) {
		issues.push(
			issue(
				"CATALOG_HEX_INVALID",
				`${context}.${key} must be nonempty lowercase even-length hex`,
				location,
			),
		);
		return undefined;
...
function optionalLowerHex(
	...
	if (
		...
		issues.push(
			issue(
				"CATALOG_HEX_INVALID",
				`${context} must be lowercase even-length hex`,
				location,
			),
		);
		return undefined;
	...
	if (typeof raw !== "number" || !Number.isSafeInteger(raw) || raw < 0) {
		issues.push(
			issue(
				...
				`${context} must be a finite nonnegative safe integer`,
				location,
			),
		);
		return undefined;
...
function stringArray(
	...
	if (!Array.isArray(raw)) {
		issues.push(
			issue("CATALOG_STRING_ARRAY_INVALID", `${context} must be an array`),
		);
		return [];
	}
	const output: string[] = [];
...
function oracleSets(
	...
	if (!Array.isArray(raw) || raw.length === 0) {
		issues.push(
			issue(
				"CATALOG_SUFFICIENT_SETS_INVALID",
				`${context} must be a nonempty array`,
				location,
			),
		);
		return undefined;
		...
		if (
			...
			issues.push(
				issue(
					...
					`${context}.${key} must be null or finite nonnegative`,
					location,
				),
			);
			continue;
...
function assignNullableSignedFinite<T extends object>(
	raw: Record<string, unknown>,
	target: T,
	keys: readonly string[],
	context: string,
	...
	for (const key of keys) {
		...
		if (
			...
			issues.push(
				issue(
					"CATALOG_SIGNED_NUMBER_INVALID",
					`${context}.${key} must be null or finite`,
					location,
				),
			);
			continue;
	...
	for (const key of keys) {
		...
		if (
			...
			issues.push(
				issue(
					"CATALOG_STRING_INVALID",
					`${context}.${key} must be null or nonempty string`,
					location,
				),
			);
			continue;
