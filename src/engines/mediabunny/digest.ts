/**
 * src/engines/mediabunny/digest.ts — frame normalization + sha256 digest for the reference engine.
 *
 * BYTE-FOR-BYTE identical to src/engines/platform/digest.ts and the rule documented in
 * INTERNAL_API.md (oracles `digestFrame`) + engine.ts §62-74. Kept self-contained under the
 * mediabunny dir (the task scopes this agent's writes here) so reference digests are comparable to
 * golden data and to every other engine's digests.
 *
 * Normalization rule:
