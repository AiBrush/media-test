/**
 * src/engines/remotion-webcodecs/digest.ts — frame normalization + sha256 digest.
 *
 * BYTE-FOR-BYTE identical to src/engines/platform/digest.ts / src/engines/mediabunny/digest.ts and
 * the rule documented in INTERNAL_API.md (oracles `digestFrame`) + engine.ts §62-74. Kept
 * self-contained under this engine's dir (the task scopes this agent's writes to
 * src/engines/remotion-webcodecs/) so its decoded-frame digests line up with golden data and with
 * every other engine's digests.
 *
 * Normalization rule:
