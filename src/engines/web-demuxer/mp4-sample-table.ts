/**
 * src/engines/web-demuxer/mp4-sample-table.ts
 *
 * Narrow helper for the huge progressive ISO-BMFF demux cells. web-demuxer exposes packet enumeration
 * through its FFmpeg/WASM packet stream, which can stall this browser harness on the largest local
 * fixtures. For faststart MP4/MOV assets, the suite's demux oracle only needs packet table fields that
 * are already present in the moov sample tables, so this helper reads that real metadata over HTTP
 * Range and derives packet rows from stsz/stts/ctts/stss.
 *
...
export interface ProgressiveMp4SampleTableDemux {
...
export async function readProgressiveMp4PacketTableFromMoov(url: string): Promise<PacketInfo[]> {
  const moov = await readMoovBox(url);
  return sampleTablesFromMoov(moov).packets;
}
