...
import type {
	...
	RemuxTrackEvidence,
} from "../src/features/remux/types.ts";
...
describe("transcode exhaustive calibration bounds", () => {
	test("every graceful edge publishes an explicit negative or boundary execution contract", () => {
		const graceful = transcodeScenarios.filter((scenario) => scenario.oracles.includes("graceful-failure"));
		expect(graceful.length).toBeGreaterThan(0);
		for (const scenario of graceful) {
			const contract = robustnessContractFromOptions(scenario.options);
			expect(contract, scenario.id).toBeDefined();
			expect(["negative", "boundary"]).toContain(contract?.inputClass);
			expect(contract?.returnedOutputCheck).toBe("media-structure");
		}
	});
	test("VP9, 2 Mbps AVC, and 1 fps rows expose their measured semantic tolerances", () => {
		const vp9 = transcodeScenarios.find(
			(entry) => entry.id === "transcode/h264_to_vp9_webm",
		)!;
		const lowBitrate = transcodeScenarios.find(
			(entry) => entry.id === "transcode/h264_bitrate_2mbps",
		)!;
		const twoPass = transcodeScenarios.find(
			(entry) => entry.id === "transcode/h264_two_pass_bitrate",
		)!;
		const oneFps = transcodeScenarios.find(
			(entry) => entry.id === "transcode/extreme_fps_1",
		)!;
		...
		expect(twoPass.revision).toBe(3);
		expect(twoPass.options.video).toMatchObject({
			codec: "h264",
			bitrate: 2_000_000,
			maxAverageBitrate: 2_600_000,
			quality: { metric: "ssim-luma-v1", minimumMean: 0.93, samples: 8 },
		...
		expect(twoPass.tolerances?.ssimMin).toBe(0.93);
		expect(twoPass.oracles).toEqual([
			"ssim-psnr",
			"average-bitrate",
			"playback-smoke",
		]);
	...
	test("rotation SSIM uses the calibrated lossy transform floor across every quarter-turn", () => {
		for (const id of [
			"h264_rotate_180",
			"h264_rotate_90_dimswap",
			"h264_rotate_270_dimswap",
		]) {
			expect(
				transcodeScenarios.find((entry) => entry.id === `transcode/${id}`)
					?.tolerances?.ssimMin,
				id,
			).toBe(0.95);
		}
...
describe("ssim-luma-v1 public-contract sampling", () => {
	...
	test("sorts/deduplicates real PTS and includes both presentation endpoints", () => {
		expect(
			endpointInclusiveQualitySamplePts([100, 50, 0, 75, 50, 25], 3),
		).toEqual([0, 50, 100]);
		expect(
			endpointInclusiveQualitySamplePts(
				new Float64Array([100, 0, 25, 50, 75]),
				5,
			),
	...
	test("uses the nearest real midpoint and resolves exact ties toward the earlier PTS", () => {
		expect(endpointInclusiveQualitySamplePts([0, 40, 60, 100], 1)).toEqual([
			40,
		]);
	...
	test("keeps irregular interior selections unique while retaining a distant last frame", () => {
		expect(endpointInclusiveQualitySamplePts([0, 1, 2, 3, 100], 4)).toEqual([
			0, 2, 3, 100,
		]);
		expect(
			endpointInclusiveQualitySamplePts([0, 1, 49, 51, 52, 100], 4),
		).toEqual([0, 49, 52, 100]);
		expect(
			new Set(endpointInclusiveQualitySamplePts([0, 1, 2, 3, 100], 4)).size,
		).toBe(4);
	});
	...
	test("returns every available real PTS and rejects malformed timestamp/sample requests", () => {
		...
		expect(() =>
			endpointInclusiveQualitySamplePts([0, Number.NaN], 1),
		).toThrow();
	});
});
...
describe("omitted AAC preservation", () => {
	test("accepts exact access units/timing and at most one observed container tick of A/V skew", () => {
		expect(
			evaluateOmittedAacPreservation(omittedAacProgram(), omittedAacProgram()),
		).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_OMITTED_AAC_PRESERVED",
		});
		expect(
			evaluateOmittedAacPreservation(
				omittedAacProgram(),
			...
			state: "VERDICT",
			verdict: "PASS",
			measurements: { containerTickToleranceUs: 1 },
		});
	});
	...
	test("rejects shifted AAC PTS through both normalized timing and A/V boundary evidence", () => {
		expect(
			...
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_OMITTED_AAC_TIMESTAMP_MISMATCH",
		});
		expect(
			evaluateOmittedAacPreservation(
				omittedAacProgram(),
			...
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_OMITTED_AAC_AV_SKEW_MISMATCH",
		});
	});
	...
	test("rejects altered AAC access-unit payload bytes", () => {
		expect(
			...
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_OMITTED_AAC_PAYLOAD_MISMATCH",
		});
	});
});

describe("REQ-FEAT-20 effect-aware transform oracles", () => {
	test("resize contracts use half-pixel bilinear sampling for RGB and alpha", () => {
		const contract = requiredTransform("vp9_alpha_to_vp9_keepalpha");
		...
		const smallContract = defineTranscodeTransformContract({
			steps: [
				{ kind: "resize", width: 3, height: 3, filter: "bilinear" },
				{ kind: "preserve-alpha" },
			],
			signal: { alphaMode: "straight" },
			tolerance: contract.tolerance,
			timestampToleranceUs: contract.timestampToleranceUs,
			allowAlternatePixelMapping: false,
		});
		const resized = applyTranscodeTransform(source, smallContract);

		expect([resized.width, resized.height]).toEqual([3, 3]);
		expect([...resized.data.slice(4 * 4, 5 * 4)]).toEqual([50, 50, 25, 139]);
	});

	test("resize contract dimensions must remain positive integers", () => {
		const contract = requiredTransform("vp9_alpha_to_vp9_keepalpha");
		const defineResize = (width: number, height: number) =>
			defineTranscodeTransformContract({
				steps: [{ kind: "resize", width, height, filter: "bilinear" }],
				signal: {},
				tolerance: contract.tolerance,
				timestampToleranceUs: contract.timestampToleranceUs,
				allowAlternatePixelMapping: false,
		...
		expect(() => defineResize(0, 240)).toThrow("positive integers");
		expect(() => defineResize(320, 240.5)).toThrow("positive integers");
		expect(() => defineResize(320, 240)).not.toThrow();
	});

	test("lossy VPx alpha admits two 8-bit values but rejects the adjacent three-value mutation", () => {
		for (const id of [
			"vp9_alpha_to_vp8_keepalpha",
			"vp9_alpha_to_vp9_keepalpha",
		]) {
			const contract = requiredTransform(id);
			expect(contract.tolerance.maxAlphaError).toBe(0.008);
		}
		const contract = requiredTransform("vp9_alpha_to_vp8_keepalpha");
		const source = pixelFrame([10, 20, 30, 100], 1, 1);
		const candidate = (alpha: number) => pixelFrame([10, 20, 30, alpha], 1, 1);
		...
		expect(
			...
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_TRANSFORM_ALPHA_MISMATCH",
		});
	});
	...
	test("display-matrix normalization preserves already presented pixels and requires identity signaling", () => {
		...
		expect(
			...
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_TRANSFORM_SIGNAL_AND_SHAPE_MATCH",
		});
		expect(
			evaluateTranscodeTransform(
				[displayed],
			...
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_TRANSFORM_SIGNALING_MISMATCH",
		});
	});
	...
	test("correct pixels cannot hide wrong authored signaling", () => {
		...
		expect(
			evaluateTranscodeTransform([source], [transformed], {}, contract),
		).toMatchObject({
			state: "UNAVAILABLE",
			status: "NA_ASSET",
			reasonCode: "TRANSCODE_TRANSFORM_SIGNALING_UNAVAILABLE",
		});
	});
	...
	test("letterbox reference scales to contain before padding, including sources larger than the target", () => {
		...
		const red4x3 = pixelFrame(
			new Array(4 * 3).fill(undefined).flatMap(() => [255, 0, 0, 255]),
			4,
			3,
		);
		const smallContract = {
		...
		const oversized = pixelFrame(
			new Array(16 * 9).fill(undefined).flatMap(() => [0, 255, 0, 255]),
			16,
			9,
		);
		expect(() =>
			applyTranscodeTransform(oversized, smallContract),
		).not.toThrow();
	});
	...
	test("color conversion preserves normalized sRGB appearance and rejects a raw no-op retag", () => {
		...
		const rawNoOpDecision = evaluateTranscodeTransform(
			[source],
			[rawNoOpRetag],
			colorSignal,
			color,
		...
		expect(
			Math.abs(rawNoOpDecision.measurements?.effectCompletionRatio ?? 1),
		).toBeLessThan(0.03);
		expect(
			rawNoOpDecision.measurements?.targetCoordinateClosenessRatio,
		).toBeLessThan(0.9);
		expect(
			evaluateTranscodeTransform(
				[source],
				[colorCandidate],
				{ ...colorSignal, colorPrimaries: "bt709" },
				color,
			),
		).toMatchObject({
			state: "VERDICT",
		...
		expect(
			evaluateTranscodeTransform([neutral], [neutral], colorSignal, color),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		const neutralRetaggedRgb = normalizedSrgbAfterRawBt2020Retag([
			128, 128, 128,
		]);
		...
		expect(neutralRetagDecision.measurements?.meanAbsoluteError).toBeLessThan(
			color.tolerance.meanAbsoluteError,
		);
	});
	...
	test("color completion is a strict semantic endpoint invariant, independent of codec-error bounds", () => {
		...
		const candidateAt = (completion: number): TranscodePixelFrame => {
			const presentation = signalOnlyRetag.map((sample, channel) =>
				Math.round(sample + completion * (sourceRgb[channel]! - sample)),
			) as [number, number, number];
			return pixelFrame([...presentation, 255], 1, 1);
		};

		for (const completion of [0.5, 0.6, 0.75, 0.89]) {
			const decision = evaluateTranscodeTransform(
				[source],
				[candidateAt(completion)],
				signal,
				color,
			);
			...
			expect(
				decision.measurements?.effectCompletionRatio,
				String(completion),
			).toBeLessThan(0.9);
			expect(
				decision.measurements?.targetCoordinateClosenessRatio,
				String(completion),
			).toBeLessThan(0.9);
		}

		const boundary = evaluateTranscodeTransform(
			[source],
			[candidateAt(0.9)],
			signal,
			color,
		...
		if (
			typeof observedBoundary !== "number" ||
			!Number.isFinite(observedBoundary)
		) {
			throw new Error(
				"color boundary did not publish a finite completion ratio",
			);
		}
		expect(
			evaluateTranscodeTransform([source], [candidateAt(0.9)], signal, {
				...color,
				minimumEffectCompletionRatio: observedBoundary,
			}),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		expect(
			evaluateTranscodeTransform([source], [candidateAt(0.9)], signal, {
				...color,
				minimumEffectCompletionRatio: observedBoundary + Number.EPSILON,
			}),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_TRANSFORM_EFFECT_INCOMPLETE",
		});
	});
	...
	test("color projection admits bounded orthogonal codec noise but rejects orthogonal work and corruption", () => {
		...
		const axis = sourceRgb.map(
			(sample, channel) => sample - noOpRgb[channel]!,
		) as [number, number, number];
			...
			pixelFrame(
				[
					...origin.map((sample, channel) =>
						Math.max(
							0,
							Math.min(255, Math.round(sample + scale * orthogonal[channel]!)),
						),
					),
					255,
				],
		...
		expect(
			Math.abs(orthogonalOnly.measurements?.effectCompletionRatio ?? 1),
		).toBeLessThan(0.03);

		const codecLikeNoise = evaluateTranscodeTransform(
			[source],
			[frameAt(sourceRgb, 0.25)],
			signal,
			color,
		);
		expect(codecLikeNoise).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		expect(
			codecLikeNoise.measurements?.effectCompletionRatio,
		).toBeGreaterThanOrEqual(0.9);
		expect(
			codecLikeNoise.measurements?.targetCoordinateClosenessRatio,
		).toBeLessThan(0.9);

		const corrupted = evaluateTranscodeTransform(
			[source],
			[frameAt(sourceRgb, 1.5)],
			signal,
			color,
		);
		expect(
			corrupted.measurements?.effectCompletionRatio,
		).toBeGreaterThanOrEqual(0.9);
		expect(corrupted).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_TRANSFORM_SEMANTIC_RESIDUAL_EXCESSIVE",
		});
	});
	...
	test("color completion accepts target-coordinate proof when decoded-sRGB projection is noise-dominated", () => {
		...
		expect(
			decision.measurements?.meanSemanticEndpointError,
		).toBeLessThanOrEqual(color.tolerance.meanAbsoluteError);
		expect(decision.measurements?.meanAbsoluteError).toBeLessThanOrEqual(
			color.tolerance.meanAbsoluteError,
		);
		expect(decision.measurements?.maximumAbsoluteError).toBeLessThanOrEqual(
			color.tolerance.maxAbsoluteError,
		);
	});
	...
	test("color alternate disposition cannot bypass physical residual, signaling, or a materially bad target mean", () => {
		...
		const alternate = evaluateTranscodeTransform(
			[source],
			[boundedAlternate],
			signal,
			color,
		...
		expect(alternate).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_TRANSFORM_ALTERNATE_VALID_MAPPING",
		});
		expect(
			alternate.measurements?.meanSemanticEndpointError,
		).toBeLessThanOrEqual(color.tolerance.meanAbsoluteError);
		...
		expect(corrupted.measurements?.meanSemanticEndpointError).toBeGreaterThan(
			color.tolerance.meanAbsoluteError,
		);
		expect(corrupted.measurements?.meanAbsoluteError).toBeGreaterThan(
			Math.max(
				color.tolerance.meanAbsoluteError * 4,
				(corrupted.measurements?.meanReferenceSourceDelta ??
	...
	test("tone-map and depth requests require both pixels and the full authored signal", () => {
		const source = pixelFrame(
			[230, 30, 20, 255, 20, 190, 35, 255, 25, 40, 220, 255, 190, 130, 15, 255],
			2,
			2,
		);
		...
		expect(
			evaluateTranscodeTransform([source], [source], { bitDepth: 8 }, depth),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_TRANSFORM_SIGNALING_MISMATCH",
		...
		const normalizedHdrPresentation = pixelFrame(
			[
				240, 210, 170, 255, 180, 200, 245, 255, 220, 100, 60, 255, 150, 230,
				120, 255,
			],
			2,
			2,
		...
		expect(
			evaluateTranscodeTransformSourceSignal(sourceSignal, tone),
		).toBeUndefined();
		expect(
			evaluateTranscodeTransformSourceSignal(
				{ ...sourceSignal, transfer: "bt709" },
				tone,
			),
		).toMatchObject({
			state: "UNAVAILABLE",
		...
		expect(
			evaluateTranscodeTransformSourceSignal({ bitDepth: 10 }, tone),
		).toMatchObject({
			state: "UNAVAILABLE",
			status: "NA_ASSET",
		...
		const toneCandidate = applyTranscodeTransform(
			normalizedHdrPresentation,
			tone,
		);
		expect(toneCandidate.bitDepth).toBe(8);
		expect([...toneCandidate.data]).toEqual([
			240, 210, 170, 255, 180, 200, 245, 255, 220, 100, 60, 255, 150, 230, 120,
			255,
		]);
		expect(
			evaluateTranscodeTransform(
		...
		const wrongPixels = pixelFrame(
			new Array(4).fill(undefined).flatMap(() => [0, 0, 0, 255]),
			2,
			2,
			8,
		);
		...
		const anchoredCandidate = anchoredSource.map((frame) =>
			applyTranscodeTransform(frame, tone),
		);
...
describe("REQ-FEAT-21 decoded audio content and explicit priming", () => {
	test("neutral WAV, FLAC, Ogg Opus, and ISO-BMFF AAC readers expose structural/timeline evidence", async () => {
		...
		const opus = readTranscodeAudioStructure(
			await fixtureBytes("opus.ogg"),
			"ogg",
		);
		expect(opus).toMatchObject({
			state: "OK",
			value: { timeline: { kind: "opus-ogg" } },
		});
		if (opus.state === "OK" && opus.value.timeline?.kind === "opus-ogg") {
			expect(opus.value.timeline.preSkipFrames).toBeGreaterThan(0);
			expect(
				opus.value.timeline.codedSampleFrames -
					opus.value.timeline.preSkipFrames -
					opus.value.timeline.endTrimFrames,
			).toBe(opus.value.timeline.presentationSampleFrames);
		}
		const aac = readTranscodeAudioStructure(
			await fixtureBytes("gapless_aac.m4a"),
			"mp4",
		);
		expect(aac).toMatchObject({
			state: "OK",
			value: { timeline: { kind: "aac-isobmff", timingSource: "edit-list" } },
		});
		const webm = readTranscodeAudioStructure(
			await fixtureBytes("tiny_vp9_360p_2s.webm"),
			"webm",
		);
		expect(webm).toMatchObject({
			state: "OK",
			value: { timeline: { kind: "opus-webm" } },
		});
		if (webm.state === "OK" && webm.value.timeline?.kind === "opus-webm") {
			expect(
				webm.value.timeline.codedSampleFrames -
					webm.value.timeline.preSkipFrames -
					webm.value.timeline.endTrimFrames,
			).toBe(webm.value.timeline.presentationSampleFrames);
		}
	});
	...
	test("lossless decoded samples must match after the declared program window", () => {
		...
		expect(
			evaluateTranscodedAudioContent(source, exact, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_AUDIO_LOSSLESS_MATCH",
		});
		const changed = audioSignal([0.25, -0.25, 0.5, -0.4], 2, 2);
		expect(
			evaluateTranscodedAudioContent(source, changed, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_LOSSLESS_CONTENT_MISMATCH",
		});

		const browserFlac = requiredAudioContract("wav_to_flac");
		const floatRounded = audioSignal(
			[0.25 + 1 / 65_536, -0.25, 0.5, -0.5],
			2,
			2,
		);
		expect(
			evaluateTranscodedAudioContent(source, floatRounded, browserFlac),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
	});
	...
	test("AAC-to-PCM admits a sparse cross-decoder transient but rejects material corruption", () => {
		...
		expect(
			...
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_AUDIO_DECODER_EQUIVALENCE_MATCH",
		});
	...
	test("legitimate AAC priming/remainder is trimmed, while excess/lost samples beyond the model fail", () => {
		...
		const candidate = audioSignal(
			[0, 0, 0, 0, 0.1, -0.1, 0.4, -0.4, 0.7, -0.7, 0.2, -0.2, 0, 0, 0, 0],
			8,
			2,
			"coded",
			timeline,
		...
		expect(
			evaluateTranscodedAudioContent(source, candidate, contract),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });

		const excess = audioSignal(
			[...candidate.samples, 0, 0],
			9,
			2,
			"coded",
			timeline,
		...
		expect(
			evaluateTranscodedAudioContent(source, excess, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_CODED_COUNT_MISMATCH",
		});

		const lost = audioSignal(
			[...candidate.samples].slice(0, -2),
			7,
			2,
			"coded",
			timeline,
		...
		expect(
			evaluateTranscodedAudioContent(source, lost, contract),
	...
	test("lossy metric rejects silence/channel destruction and never guesses missing AAC/Opus timing", () => {
		const source = audioSignal(
			[0.8, -0.2, 0.4, -0.7, -0.6, 0.5, 0.2, 0.9],
			4,
			2,
		);
		const contract = requiredAudioContract("wav_to_aac_mp4");
		...
		expect(
			evaluateTranscodedAudioContent(source, silence, contract),
		).toMatchObject({
			state: "UNAVAILABLE",
			reasonCode: "TRANSCODE_AUDIO_TIMELINE_EVIDENCE_MISSING",
		});

		const wholeProgramContract = {
			...contract,
			requireExplicitTimeline: false,
		};
		expect(
			evaluateTranscodedAudioContent(source, silence, wholeProgramContract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
	...
	test("Opus SNR gate admits the cross-encoder 13.84 dB frontier but rejects a lower neighbor", () => {
		...
		const candidateAtSnr = (snrDb: number): DecodedAudioSignal => {
			...
			const samples = sourceSamples.map(
				(sample, index) =>
					sample +
					noiseAmplitude * Math.sin((2 * Math.PI * 31 * index) / frames),
			);
		...
		expect(
			evaluateTranscodedAudioContent(source, candidateAtSnr(13.84), contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
		...
		expect(
			evaluateTranscodedAudioContent(source, candidateAtSnr(13.25), contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_LOSSY_CONTENT_MISMATCH",
		...
		const resampledSource = { ...source, resampledFromSampleRate: 44_100 };
		const candidateWithTail = (extraFrames: number): DecodedAudioSignal => {
			const samples = [...sourceSamples, ...new Array(extraFrames).fill(0)];
			const sampleFrames = frames + extraFrames;
			return audioSignal(samples, sampleFrames, 1, "coded", {
				kind: "opus-ogg",
				codedSampleFrames: sampleFrames,
				preSkipFrames: 0,
				endTrimFrames: 0,
				finalGranulePosition: sampleFrames,
		...
		expect(
			evaluateTranscodedAudioContent(
				resampledSource,
				candidateWithTail(241),
				contract,
			),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_EXCESS_SAMPLES",
		});
		expect(
			evaluateTranscodedAudioContent(source, candidateWithTail(1), contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_EXCESS_SAMPLES",
	...
	test("whole-program granule timing trims coded decoder tail padding without hiding truncation", () => {
		...
		const padded = audioSignal(
			[0.2, -0.2, 0.4, -0.4, 0.75],
			5,
			1,
			"coded",
			timeline,
		...
		expect(
			evaluateTranscodedAudioContent(source, padded, contract),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		const truncated = audioSignal([0.2, -0.2, 0.4], 3, 1, "coded", timeline);
		expect(
			evaluateTranscodedAudioContent(source, truncated, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_DECODED_PROGRAM_TRUNCATED",
		});
	});
	...
	test("MP3-in-MP4 admits only the documented sub-millisecond sample-time rounding band", () => {
		...
		const rounded = audioSignal(
			[0.2, -0.2, 0.4, -0.4, ...new Array(16).fill(0)],
			20,
			1,
		);
		expect(
			evaluateTranscodedAudioContent(source, rounded, contract),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		const excessive = audioSignal(
			[0.2, -0.2, 0.4, -0.4, ...new Array(33).fill(0)],
			37,
			1,
		);
		expect(
			evaluateTranscodedAudioContent(source, excessive, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_EXCESS_SAMPLES",
	...
	test("AAC-in-MP4 admits sub-millisecond edit-list timescale rounding but not coded-frame drift", () => {
		const sourceSamples = Array.from({ length: 64 }, (_, index) =>
			index % 2 === 0 ? 0.25 : -0.25,
		);
		...
		const contract = requiredAudioContract("wav_to_aac_mp4");
		expect(
			evaluateTranscodedAudioContent(source, candidate(32), contract),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		expect(
			evaluateTranscodedAudioContent(source, candidate(33), contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_EXCESS_SAMPLES",
	...
	test("the muxed stereo-to-mono row scores the declared channel transform, not metadata alone", () => {
		const source = audioSignal(
			[0.8, -0.2, 0.4, -0.6, -0.6, 0.2, 0.2, 0.8],
			4,
			2,
		);
		const timeline = {
		...
		expect(
			evaluateTranscodedAudioContent(source, mono, contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_AUDIO_LOSSY_CONTENT_MATCH",
		...
		const longSource = audioSignal(
			new Array(128).fill([0.8, -0.2]).flat(),
			128,
			2,
		);
		const candidate = (frames: number) =>
			audioSignal(new Array(frames).fill(0.3), frames, 1, "presentation", {
				...timeline,
				codedSampleFrames: frames,
				presentationSampleFrames: frames,
			});
		expect(
			evaluateTranscodedAudioContent(longSource, candidate(48), contract),
		).toMatchObject({ state: "VERDICT", verdict: "PASS" });
		expect(
			evaluateTranscodedAudioContent(longSource, candidate(31), contract),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_AUDIO_LOST_SAMPLES",
		});
	});
	...
	test("every registered audio-content contract is live on its scenario", () => {
		for (const id of transcodeAudioContractScenarioIds()) {
			...
			expect(scenario, id).toBeDefined();
			expect(scenario!.oracles, id).toContain("property-invariant");
			expect(optionsOf(scenario!).invariant, id).toBe(
				"transcode-audio-content",
			);
		}
	});
});
...
describe("REQ-FEAT-22 ABR renditions form a switchable set", () => {
	...
	test("bitrate, duration, timebase, random access, and bidirectional adjacent switching all gate", () => {
		...
		expect(
			evaluateAbrSwitchability(contract, description, evidence, switches),
		).toMatchObject({
			state: "VERDICT",
			verdict: "PASS",
			reasonCode: "TRANSCODE_ABR_SWITCHABLE_SET",
		});
		expect(
			evaluateAbrSwitchability(contract, undefined, evidence),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_ABR_DESCRIPTION_MISSING",
		});
		expect(
			evaluateAbrSwitchability(contract, description, evidence),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_ABR_SWITCH_DECODE_EVIDENCE_MISSING",
		});

		const bitrateWrong = [
			{ ...evidence[0]!, bitrateBps: 200_000 },
			evidence[1]!,
		];
		expect(
			evaluateAbrSwitchability(contract, description, bitrateWrong, switches),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_ABR_BITRATE_BAND_MISMATCH",
		});
		...
		const keyframeWrong = [
			...
			{
				...evidence[1]!,
				samples: evidence[1]!.samples.map((sample, index) => ({
					...sample,
					keyframe: index === 0,
				})),
			},
		...
		expect(
			evaluateAbrSwitchability(contract, description, keyframeWrong, switches),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_ABR_RANDOM_ACCESS_MISALIGNED",
		});

		const decodedGap = switches.map((attempt, index) =>
			index === 0
				? { ...attempt, targetFirstPtsUs: attempt.targetFirstPtsUs + 10_000 }
				: attempt,
		);
		expect(
			evaluateAbrSwitchability(contract, description, evidence, decodedGap),
		).toMatchObject({
			state: "VERDICT",
			verdict: "FAIL",
			reasonCode: "TRANSCODE_ABR_DECODED_SWITCH_GAP",
		});
	});
	...
	test("independent quality representation difference remains a valid switchable set", () => {
		const evidence = [
			...
			{
				...abrEvidence("low", 640, 360, 1_000_000),
				quality: transcodeVerdict(
					"PASS",
					"ORACLE_REPRESENTATION_DIFF",
					"alternate valid encode",
				),
			},
		];
	...
	test("the actual MP4 collector measures video samples rather than file-count proxies", async () => {
		...
		const result = collectAbrRenditionEvidence(
			"tiny",
			{ bytes, mime: "video/mp4", container: "mp4" },
		...
		expect(TRANSCODE_ABR_CONTRACT.renditions.map((entry) => entry.id)).toEqual([
			"1080p",
			"720p",
			"480p",
			"360p",
		...
		const scenario = transcodeScenarios.find(
			(entry) => entry.id === "transcode/fanout_h264_abr_ladder",
		)!;
...
describe("single-output average video bitrate correctness gate", () => {
	...
	const playback: OracleOutcome = {
		...
		verdict: "PASS",
		reasonCode: "PLAYBACK_OK",
		detail: "played",
	};

	test("4 Mbps/high-SSIM fails rate, true-rate/low-SSIM fails quality, and only both pass", () => {
		const fourMbps = averageBitrateOutcome(
			evaluateAverageVideoBitrate(contract, {
		...
		expect(
			reduceOracleOutcomes([fourMbps, quality("PASS", 0.99), playback]).status,
		).toBe("FAIL");

		const twoMbps = averageBitrateOutcome(
			evaluateAverageVideoBitrate(contract, {
		...
		expect(
			reduceOracleOutcomes([twoMbps, quality("FAIL", 0.8), playback]).status,
		).toBe("FAIL");
		expect(
			reduceOracleOutcomes([twoMbps, quality("PASS", 0.95), playback]).status,
		).toBe("PASS");
	});

	test("neutral collection uses video sample payload and presentation span, excluding file overhead", async () => {
		const bytes = await fixtureBytes("tiny_h264_360p_2s.mp4");
		const evidence = collectAverageVideoBitrateEvidence({
			bytes,
			mime: "video/mp4",
			container: "mp4",
		});
		...
		expect(evidence.value.videoSamplePayloadBytes).toBeLessThan(
			bytes.byteLength,
		);
		...
		const measuredBps =
			(evidence.value.videoSamplePayloadBytes * 8 * 1_000_000) /
...
describe("REQ-FEAT-23 real output-bound round trip", () => {
	test("leg two consumes leg one exact bytes and the final reference stays original", async () => {
		...
		expect(seen.get(TRANSCODE_ROUNDTRIP_CONTRACT.leg2ScenarioId)).toEqual(
			new Uint8Array([1, 2, 3, 9]),
		);
		expect(result.value.leg2ConsumedSha256).toBe(result.value.leg1OutputSha256);
		expect(result.value.finalReferenceSha256).toBe(
			result.value.originalSourceSha256,
		);
		expect(
			assessTranscodeRoundTripProvenance(
				TRANSCODE_ROUNDTRIP_CONTRACT,
	...
	test("perturbing leg one changes leg two source, and a stale fixed input digest fails", async () => {
		...
		const run = async (marker: number) =>
			executeTranscodeRoundTrip(
				TRANSCODE_ROUNDTRIP_CONTRACT,
				media([4, 5, 6], "mp4"),
				async (scenarioId, input) => {
					const bytes = input.materialize();
					if (scenarioId === TRANSCODE_ROUNDTRIP_CONTRACT.leg2ScenarioId)
						leg2Inputs.push(sha256Hex(bytes));
					return {
						consumedInputSha256: sha256Hex(bytes),
						output:
							scenarioId === TRANSCODE_ROUNDTRIP_CONTRACT.leg1ScenarioId
		...
		const stale = await executeTranscodeRoundTrip(
			...
			async (scenarioId, input) => ({
				consumedInputSha256:
					scenarioId === TRANSCODE_ROUNDTRIP_CONTRACT.leg2ScenarioId
						? "0".repeat(64)
						: input.sha256,
				output: media(
					[7, 8, 9],
	...
	test("leg two declares the composed invariant instead of claiming a fixed stand-in is cumulative", () => {
		const leg2 = transcodeScenarios.find(
			(entry) => entry.id === TRANSCODE_ROUNDTRIP_CONTRACT.leg2ScenarioId,
		)!;
...
describe("REQ-FEAT-24 correctness-gated metrics and honest thresholds", () => {
	const passEvidence = [
		{
			oracle: "ssim-psnr" as const,
			state: "VERDICT" as const,
			verdict: "PASS" as const,
			reasonCode: "ORACLE_MATCH",
		},
		{
			oracle: "playback-smoke" as const,
			state: "VERDICT" as const,
			verdict: "PASS" as const,
			reasonCode: "ORACLE_MATCH",
		},
	...
	test("every admitted rate names observed components and its PASS/DIFF verdict", () => {
		...
		const rate = makeTranscodeRateEvidence({
			metric: "framesPerSec",
			numerator: {
				name: "output presentation frames",
				value: 60,
				unit: "frame",
				source: "neutral-output-sample-table",
			},
			denominator: {
				name: "measured operation wall",
				value: 2,
				unit: "second",
		...
		expect(
			admitTranscodeMetrics(contract, passEvidence, {}, [rate]),
		).toMatchObject({
			state: "ADMITTED",
			associatedVerdict: "PASS",
			rates: [{ value: 30 }],
		...
		const estimated = {
			...
			numerator: {
				...rate.numerator,
				source: "source nominal fps times golden duration estimate",
			},
		} as unknown as typeof rate;
		expect(
			admitTranscodeMetrics(contract, passEvidence, {}, [estimated]),
		).toMatchObject({
			state: "BLOCKED",
			decision: {
				state: "UNAVAILABLE",
				reasonCode: "TRANSCODE_RATE_NUMERATOR_ESTIMATED",
			},
		});
	});
	...
	test("missing mandatory evidence cannot publish; a representation difference publishes as PASS", () => {
		...
		expect(
			admitTranscodeMetrics(contract, passEvidence.slice(0, 1), {}, []),
		).toMatchObject({
			state: "BLOCKED",
			decision: {
				state: "UNAVAILABLE",
				reasonCode: "TRANSCODE_METRIC_MANDATORY_EVIDENCE_MISSING",
			},
		});
		expect(
			admitTranscodeMetrics(
				contract,
				[...passEvidence, passEvidence[0]!],
				{},
				[],
			),
		).toMatchObject({
			...
			decision: {
				state: "ERROR",
				reasonCode: "TRANSCODE_METRIC_ORACLE_EVIDENCE_DUPLICATE",
			},
		});
		...
		expect(admitTranscodeMetrics(contract, diffEvidence, {}, [])).toMatchObject(
			{ state: "ADMITTED", associatedVerdict: "PASS" },
		);

		const allowed = defineTranscodeMetricAdmissionContract({
			mandatoryOracles: ["ssim-psnr", "playback-smoke"],
			allowedDiffs: [
				{ oracle: "ssim-psnr", reasonCodes: ["ORACLE_REPRESENTATION_DIFF"] },
			],
			thresholds: [],
		});
		expect(admitTranscodeMetrics(allowed, diffEvidence, {}, [])).toMatchObject({
			state: "ADMITTED",
			associatedVerdict: "PASS",
		});
	});
	...
	test("changing a gating threshold changes the verdict; advisory thresholds never pretend to gate", () => {
		...
		expect(
			...
			decision: {
				state: "VERDICT",
				verdict: "FAIL",
				reasonCode: "TRANSCODE_GATING_THRESHOLD_FAILED",
			},
		});
		expect(
			admitTranscodeMetrics(
				contractAt(0.99, "advisory"),
				passEvidence,
				{ ssimScore: 0.95 },
				[],
			),
			...
			state: "ADMITTED",
			advisoryThresholds: [{ id: "ssim", passed: false }],
		});
	});

	test("scenario thresholds no longer expose ignored PSNR gates or inferred encode/decode rates", () => {
		expect(
			transcodeScenarios.some(
				(scenario) => scenario.tolerances.psnrMinDb !== undefined,
			),
		).toBe(false);
		expect(
			transcodeScenarios.some(
				(scenario) =>
					scenario.metrics.includes("encodeFps") ||
					scenario.metrics.includes("decodeFps"),
			),
		).toBe(false);
		const contract = transcodeMetricAdmissionContract({
			oracles: ["ssim-psnr"],
			ssimMin: 0.97,
		});
...
function pixelFrame(
	...
	const data =
		bitDepth <= 8
			...
			: new Uint16Array(
					values.map((value) =>
						Math.round((value * (2 ** bitDepth - 1)) / 255),
					),
				);
...
function normalizedSrgbAfterRawBt2020Retag(
	...
	return matrix.map((row) =>
		Math.round(srgbOetf(row[0] * red + row[1] * green + row[2] * blue) * 255),
	) as [number, number, number];
}
...
function abrSwitchEvidence(
	description: AbrRenditionSetDescription,
): AbrSwitchDecodeEvidence[] {
...
function optionsOf(
	scenario: (typeof transcodeScenarios)[number],
): Record<string, unknown> {
	return scenario.options as Record<string, unknown>;
}

function omittedAacProgram(
	options: {
		dropAudio?: boolean;
	...
	const video: RemuxTrackEvidence = {
		...
		timescale: 1_000_000,
		samples: [
			sample([0x65], 0, 1_000, true),
			sample([0x41], 1_000, 1_000, false),
		],
	};
	...
	const audio: RemuxTrackEvidence = {
		...
		samples: [
			...
			sample(
				[3, options.alterPayload ? 9 : 4],
				600 + shift + (options.shiftMiddleAudioUs ?? 0),
				700,
			),
...
async function fixtureBytes(file: string): Promise<Uint8Array> {
	return new Uint8Array(
		await Bun.file(`${ROOT}/fixtures/media/${file}`).arrayBuffer(),
	);
}
