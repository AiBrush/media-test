...
function sourceFile(
	name: string,
	contents: string,
	overrides: Partial<SourceFileRecord> = {},
): SourceFileRecord {
...
function row(
	...
	overrides: Partial<ScenarioSourceRow> = {},
): ScenarioSourceRow {
	return {
...
test("fixed-layout audio-DSP rows keep arbitrary same-codec real files out of the pool", () => {
	...
	const selections = candidatesForRun(
		[fixed],
		new Map([[fixed.id, sourceRow]]),
	);
	expect(
		selections.get(fixed.id)?.map((selection) => selection.isBaked),
	).toEqual([true]);
});
...
test("VP9 alpha exhaustive selection retains the baked input and all three curated real sources", async () => {
	...
	const selectedScenario = decodeSeekScenarios.find(
		(entry) => entry.id === "decode-seek/decode_vp9_alpha",
	);
	if (!selectedScenario)
		throw new Error("VP9 alpha decode scenario is missing");
	const pool = findScenarioPool(
		buildSelectionManifest({
	...
	expect(
		pool.candidates.map((candidate) => candidate.selectedFile).sort(),
	).toEqual(["01.webm", "02.webm", "03.webm", "vp9_alpha.webm"]);
...
test("rotate-normalize rejects shape-compatible files that do not declare the authored display matrix", () => {
	...
	const real = sourceFile(
		"01.mp4",
		"shape-compatible but orientation-free bytes",
	);
	const sourceRow = row([real], { scenarioId: fixed.id });
...
test("metadata fixture-semantic rows are baked-only while the family retains the exact 25-row/76-member pool", async () => {
	...
	if (parsedCatalog.state !== "VALID") {
		throw new Error(
			parsedCatalog.issues.map((issue) => issue.detail).join("; "),
		);
	}
	...
	expect(
		manifest.pools.reduce((sum, pool) => sum + pool.candidates.length, 0),
	).toBe(76);
	for (const id of fixtureBound) {
		const pool = findScenarioPool(manifest, id);
		expect(
			pool?.candidates.map((candidate) => candidate.kind),
			id,
		).toEqual(["baked"]);
		expect(pool?.rejections, id).toHaveLength(3);
		expect(
			pool?.rejections.every(
				(rejection) =>
					rejection.reasonCode === "SOURCE_SEMANTICS_FIXTURE_BOUND",
			),
			id,
		).toBe(true);
	}
...
test("performance workload envelopes produce the exact revision-2 canonical pools", async () => {
	...
	expect(
		manifest.pools.reduce((sum, pool) => sum + pool.rejections.length, 0),
	).toBe(54);
	const poolCounts = manifest.pools.reduce<Record<number, number>>(
		(counts, pool) => {
			counts[pool.candidates.length] =
				(counts[pool.candidates.length] ?? 0) + 1;
			return counts;
		},
		{},
	);
	...
	for (const id of [
		...
		const pool = findScenarioPool(manifest, id);
		expect(
			pool?.candidates.map((candidate) => candidate.selectedFile),
			id,
		).toEqual(["h264_1080p_30s.mp4"]);
		expect(pool?.rejections, id).toHaveLength(3);
	}

	for (const pool of manifest.pools.filter((entry) =>
		entry.scenarioId.startsWith("performance/size-ladder-"),
	)) {
...
): ValidatedScenarioSourceCatalog {
...
function bakedManifest(
	assets: Array<{ id: string; contents: string; source?: string }> = [
		{ id: "baked.mp4", contents: "baked bytes" },
	],
...
test("full-range no-op selection admits only candidates with the authored full duration", () => {
	...
	const candidate = (name: string, durationSec: number) =>
	...
	const contractRow = (file: SourceFileRecord) =>
		row([file], {
			scenarioId: noop.id,
			requires: {
				container: "webm",
	...
	expect(
		assessCandidateEligibility(noop, contractRow(exact), exact).eligible,
	).toBe(true);
	const rejected = assessCandidateEligibility(
		noop,
		contractRow(unrelated),
		unrelated,
	);
...
test("source envelopes admit arbitrary matching assets at inclusive boundaries and fail closed", () => {
	...
	const candidate = (name: string, overrides: Partial<SourceFileRecord> = {}) =>
		sourceFile(name, name, {
			container: "webm",
			videoCodecs: ["vp9"],
			audioCodecs: ["opus"],
	...
	const eligibility = (file: SourceFileRecord) =>
		assessCandidateEligibility(
			boundedScenario,
	...
	for (const [file, field] of [
		...
		if (!result.eligible) {
			expect(result.rejection.reasonCode).toBe(
				"CANDIDATE_INPUT_CONTRACT_MISMATCH",
			);
			expect(result.rejection.detail).toContain(field);
		}
	}

	const catalogOnly = candidate("catalog-only-bound.webm", {
		durationSec: 107,
	});
	const catalogResult = assessCandidateEligibility(
		scenario({
			id: "transcode/catalog-envelope-contract",
			family: "transcode",
			op: "transcode",
		}),
		row([catalogOnly], {
	...
	const matchingRow = row([matching], {
		scenarioId: boundedScenario.id,
		requires,
	});
	const candidateCatalog = catalogFromRows([matchingRow]);
	const candidateBaked = bakedManifest([
		{ id: "baked.webm", contents: "same baked bytes" },
	]);
	const boundedPool = findScenarioPool(
		buildSelectionManifest({
	...
	const unboundedPool = findScenarioPool(
		buildSelectionManifest({
	...
	expect(
		boundedPool.candidates.map((entry) => entry.candidateIdentity),
	).toEqual(unboundedPool.candidates.map((entry) => entry.candidateIdentity));
	expect(boundedPool.eligiblePoolDigest).not.toBe(
		unboundedPool.eligiblePoolDigest,
	);
});

test("audio-content envelopes exclude silent, unknown, and below-floor candidates without pinning filenames", () => {
	const contentScenario = scenario({
		id: "transcode/discriminating-audio-source",
		family: "transcode",
		op: "transcode",
	...
	const eligibility = (file: SourceFileRecord) =>
		assessCandidateEligibility(
			contentScenario,
			row([file], { scenarioId: contentScenario.id }),
			file,
		);

	expect(
		eligibility(
			sourceFile("audible-a.mp4", "a", { audioMeanVolumeDb: -79.999 }),
		).eligible,
	).toBe(true);
	expect(
		eligibility(sourceFile("audible-b.mp4", "b", { audioMeanVolumeDb: -14 }))
			.eligible,
	).toBe(true);
	for (const [file, expected] of [
		[sourceFile("silent.mp4", "silent", { audioMeanVolumeDb: -91 }), "-91dB"],
		[
			sourceFile("unknown.mp4", "unknown", { audioMeanVolumeDb: null }),
			"unknown",
		],
		[sourceFile("unmeasured.mp4", "unmeasured"), "unknown"],
	] as const) {
		const result = eligibility(file);
		expect(result.eligible).toBe(false);
		if (!result.eligible) {
			...
			expect(result.rejection.detail).toContain("audio mean volume");
			expect(result.rejection.detail).toContain(expected);
		}
	}
});

test("geometry envelopes exclude exact identity dimensions while admitting neighboring shapes", () => {
	const geometryScenario = scenario({
		id: "transcode/discriminating-resize-source",
		family: "transcode",
		op: "transcode",
		candidateEnvelope: { excludeDimensions: [{ width: 320, height: 240 }] },
	});
	const eligibility = (file: SourceFileRecord) =>
		assessCandidateEligibility(
			geometryScenario,
			row([file], { scenarioId: geometryScenario.id }),
			file,
		);

	expect(
		eligibility(
			sourceFile("larger.webm", "larger", { width: 640, height: 480 }),
		).eligible,
	).toBe(true);
	expect(
		eligibility(
			sourceFile("neighbor.webm", "neighbor", { width: 320, height: 241 }),
		).eligible,
	).toBe(true);
	const excluded = eligibility(
		sourceFile("identity.webm", "identity", { width: 320, height: 240 }),
	);
	...
	if (!excluded.eligible)
		expect(excluded.rejection.detail).toContain("320x240px");
});
...
test("tracked large-rung envelopes keep baked when regenerated catalog rows have no bounds", () => {
	...
	for (const current of currentRungs) {
		const selectedScenario = transcodeScenarios.find(
			(entry) => entry.id === current.scenarioId,
		);
		if (!selectedScenario)
			throw new Error(`missing tracked scenario '${current.scenarioId}'`);
		expect(selectedScenario).toMatchObject({
			revision: 2,
			candidateEnvelope: exactLargeEnvelope,
		});
		const sourceRow = row(
			current.files.map((file) =>
				sourceFile(file.file, `current ${file.file}`, {
		...
		const manifest = buildSelectionManifest({
			...
			catalog: catalogFromRows([sourceRow]),
			bakedManifest: bakedManifest([
				{ id: current.bakedId, contents: `verified baked ${current.bakedId}` },
			]),
		});
		const pool = findScenarioPool(manifest, current.scenarioId);
		if (!pool)
			throw new Error(`missing candidate pool '${current.scenarioId}'`);
...
test("every tracked demux size row filters real candidates by its independent workload envelope", () => {
	const cases = [
		{
			scenarioId: "demux/size_micro_micro_h264_1frame",
			bakedId: "micro_h264_1frame.mp4",
			container: "mp4",
			videoCodec: "h264",
		...
		{
			scenarioId: "demux/size_micro_micro_audio_short",
			bakedId: "micro_audio_short.m4a",
			container: "mp4",
		...
		{
			scenarioId: "demux/size_tiny_tiny_h264_360p_2s",
			bakedId: "tiny_h264_360p_2s.mp4",
			container: "mp4",
			videoCodec: "h264",
			audioCodec: "aac",
			width: 640,
		...
		{
			scenarioId: "demux/size_tiny_tiny_vp9_360p_2s",
			bakedId: "tiny_vp9_360p_2s.webm",
			container: "webm",
			videoCodec: "vp9",
			audioCodec: "opus",
			width: 640,
		...
		},
		{
			...
			bakedId: "huge_h264_1080p_600s.mov",
			container: "mov",
			videoCodec: "h264",
			audioCodec: "aac",
			width: 1920,
		...
		},
		{
			...
			bakedId: "massive_h264_1080p_2h.mp4",
			container: "mp4",
			videoCodec: "h264",
			audioCodec: "aac",
			width: 1920,
	...
	for (const current of cases) {
		const selectedScenario = demuxScenarios.find(
			(entry) => entry.id === current.scenarioId,
		);
		if (!selectedScenario)
			throw new Error(`missing tracked scenario '${current.scenarioId}'`);
		const exact = sourceFile(
			`exact-${current.bakedId}`,
			`exact ${current.bakedId}`,
			{
				container: current.container,
		...
		const wrongDuration = sourceFile(
			`wrong-duration-${current.bakedId}`,
			`wrong ${current.bakedId}`,
			{
				container: current.container,
				videoCodecs: current.videoCodec ? [current.videoCodec] : [],
				audioCodecs: current.audioCodec ? [current.audioCodec] : [],
				...
				...(current.height !== undefined ? { height: current.height } : {}),
				durationSec:
					(selectedScenario.candidateEnvelope?.maxDurationSec ??
						current.durationSec) + 0.001,
			},
		);
		const files =
		...
		expect(
			pool.candidates.map((candidate) => candidate.selectedFile).sort(),
		).toEqual([current.bakedId, exact.file].sort());
		...
		...(options.scenarioContractDigest
			? {
					scenarioContractDigests: {
						[selectedScenario.id]: options.scenarioContractDigest,
					},
				}
			: {}),
	});
...
function poolFor(
	files: SourceFileRecord[],
	options: Parameters<typeof manifestFor>[1] = {},
): ScenarioCandidatePool {
	const selectedScenario = options.scenario ?? scenario();
	const pool = findScenarioPool(
		manifestFor(files, { ...options, scenario: selectedScenario }),
		selectedScenario.id,
	);
...
describe("REQ-SEL-01 canonical validated candidate manifest", () => {
	test("row, file, and object-key order leave catalog/manifest identity unchanged", () => {
		...
		const second = row([sourceFile("03.mp4", "three")], {
			scenarioId: "probe/second",
		});
		...
		const manifestA = buildSelectionManifest({
			scenarios: [scenario()],
			catalog: a.catalog,
			bakedManifest: bakedManifest(),
		});
		const manifestB = buildSelectionManifest({
			scenarios: [scenario()],
			catalog: b.catalog,
			bakedManifest: bakedManifest(),
		});
		...
		expect(
			manifestA.pools[0]?.candidates.map(
				(candidate) => candidate.candidateIdentity,
			),
		).toEqual(
			manifestB.pools[0]?.candidates.map(
				(candidate) => candidate.candidateIdentity,
			),
		);
	...
	test("duplicate ids, paths, and in-scenario content digests are stable fatal diagnostics", () => {
		...
		expect(parsed.issues.map((issue) => issue.reasonCode)).toEqual(
	...
	test("catalog audio levels retain signed measurements and reject implausible mutations", () => {
		const measured = row([
			sourceFile("audible.mp4", "audible", { audioMeanVolumeDb: -14 }),
			sourceFile("silence.mp4", "silence", { audioMeanVolumeDb: null }),
		]);
		const valid = parseScenarioSourceCatalog(JSON.stringify(measured));
		expect(valid.state).toBe("VALID");
		if (valid.state === "VALID") {
			expect(
				valid.catalog.rows[0]?.files.map((file) => file.audioMeanVolumeDb),
			).toEqual([-14, null]);
		}

		const mutated = structuredClone(measured);
		mutated.files[0]!.audioMeanVolumeDb = -161;
		const invalid = parseScenarioSourceCatalog(JSON.stringify(mutated));
		expect(invalid.state).toBe("INVALID");
		expect(invalid.issues).toContainEqual(
			expect.objectContaining({ reasonCode: "CATALOG_AUDIO_LEVEL_INVALID" }),
		);
	});
	...
	test("source-envelope requirements are validated as ordered finite ranges", () => {
		...
		if (parsedValid.state === "VALID") {
			expect(parsedValid.catalog.rows[0]?.requires).toMatchObject(
				valid.requires,
			);
		}
		...
		expect(parsedInvalid.issues.map((issue) => issue.reasonCode)).toEqual(
			expect.arrayContaining([
	...
	test("explicit fallback retains its reason and cryptographic baked corpus identity", () => {
		...
		expect(manifest.catalogReason).toEqual({
			reasonCode: "CATALOG_HTTP_ERROR",
			detail: "HTTP 404",
		});
		...
		expect(
			manifest.pools[0]?.candidates[0]?.provenance[0]?.entity.sha256,
		).toMatch(/^[0-9a-f]{64}$/);
	});
});
...
describe("REQ-SEL-02 exact byte identity before engine use", () => {
	test("incremental SHA-256 matches FIPS vectors and an independent digest across block boundaries", async () => {
		...
		for (const length of [55, 56, 63, 64, 65, 119, 120, 127, 128, 129, 1_087]) {
			const input = Uint8Array.from(
				{ length },
				(_, index) => (index * 131 + length) & 0xff,
			);
			const expected = bytesToLowerHex(
			...
			while (offset < input.byteLength) {
				const end = Math.min(
					input.byteLength,
					offset + updateSizes[updateIndex % updateSizes.length]!,
				);
	...
	test("incremental SHA-256 snapshots resume exactly and reject malformed state", async () => {
		const input = Uint8Array.from({ length: 4_097 }, (_, index) => (index * 197 + 31) & 0xff);
		const expected = bytesToLowerHex(
			new Uint8Array(await crypto.subtle.digest("SHA-256", input)),
		);
		let hash = new Sha256();
		let offset = 0;
		for (const width of [1, 63, 64, 511, 1_024, 2_434]) {
			const end = Math.min(input.byteLength, offset + width);
			hash.update(input.subarray(offset, end));
			hash = Sha256.fromSnapshot(structuredClone(hash.snapshot()));
			offset = end;
		}
		expect(offset).toBe(input.byteLength);
		expect(hash.hex()).toBe(expected);
		expect(() => Sha256.fromSnapshot({
			...new Sha256().snapshot(),
			blockLength: 1,
	...
	test("stream admission is incremental, non-retained, and preserves an authenticated block map", async () => {
		...
		const chunks = delivered.slice();
		const released: Uint8Array[] = [];
		const result = await verifyContentStream(
			identity,
			async () =>
		...
		if (result.state === "VERIFIED_STREAM") {
			expect(result.chunkSha256).toHaveLength(
				Math.ceil(expected.byteLength / 5),
			);
			expect("bytes" in result).toBe(false);
			for (let index = 0; index < result.chunkSha256.length; index++) {
				expect(result.chunkSha256[index]).toBe(
					sha256Hex(expected.subarray(index * 5, (index + 1) * 5)),
				);
			}
		}
		expect(released).toEqual(delivered);
		expect(released.every((chunk) => chunk.every((byte) => byte === 0))).toBe(true);

		const corrupt = expected.slice();
		corrupt[7] ^= 0xff;
		const rejected = await verifyContentStream(
			identity,
			async () => new Response(corrupt).body!,
		);
		expect(rejected).toMatchObject({
			state: "REJECTED",
			issue: { reasonCode: "CORPUS_DIGEST_MISMATCH" },
		});
	});
					...
					start(controller) {
						for (
							...
							controller.enqueue(
								input.subarray(
									offset,
									Math.min(input.byteLength, offset + 256 * 1024),
								),
							);
						}
						controller.close();
	...
	test("verified bytes reach the callback, while truncation and replacement never do", async () => {
		...
		const identity = {
			logicalPath: "scenarios/probe/selection/01.mp4",
			sha256: sha256Hex(expected),
			sizeBytes: expected.byteLength,
		};
		let engineCalls = 0;
		const valid = await withVerifiedContent(
			[identity],
			async () => expected,
			(verified) => {
				engineCalls += 1;
		...
		const truncated = await withVerifiedContent(
			[identity],
			async () => expected.subarray(0, expected.length - 1),
			() => {
				engineCalls += 1;
			},
		);
		expect(truncated).toMatchObject({ state: "NA_ASSET", eligible: 0 });
		if (truncated.state === "NA_ASSET")
			expect(truncated.issues[0]?.reasonCode).toBe("CORPUS_SIZE_MISMATCH");

		const replacement = bytes("changed candidate byt");
		expect(replacement.byteLength).toBe(expected.byteLength);
		const replaced = await withVerifiedContent(
			[identity],
			async () => replacement,
			() => {
				engineCalls += 1;
		...
		if (replaced.state === "NA_ASSET")
			expect(replaced.issues[0]?.reasonCode).toBe("CORPUS_DIGEST_MISMATCH");
		expect(engineCalls).toBe(1);
	});
...
describe("REQ-SEL-03/05 order-independent canonical selection, replay, and unique-content identity", () => {
	...
	test("hashing has fixed golden vectors and selection is invariant under every file permutation", () => {
		...
		expect([...picks]).toEqual([
			[...poolFor(files).candidates].sort((left, right) =>
				left.candidateIdentity.localeCompare(right.candidateIdentity),
			)[0]!.candidateIdentity,
		]);
	});
	...
	test("adding/removing candidates follows the canonical candidate-identity minimum", () => {
		...
		if (before.state !== "SELECTED" || after.state !== "SELECTED")
			throw new Error("unexpected empty pool");
	...
	test("canonical replay rejects pool drift; recorded full candidate replays explicitly after drift", () => {
		...
		expect(
			selectCandidateFromPool(changed, {
		...
		if (replay.state === "SELECTED")
			expect(replay.candidate.candidateIdentity).toBe(
				selected.candidate.candidateIdentity,
			);
	});
		...
		expect(
			pool.candidates.every(
				(candidate) =>
					...
					candidate.probability.weight === 1,
			),
		).toBe(true);
	});
	...
	test("duplicate content is rejected, never admitted as extra probability mass", () => {
		const duplicate = sourceFile("01.mp4", "same");
		const parsed = parseScenarioSourceCatalog(
			JSON.stringify(row([duplicate, { ...duplicate, file: "02.mp4" }])),
		);
		expect(parsed.state).toBe("INVALID");
		expect(
			parsed.issues.some(
				(issue) => issue.reasonCode === "CATALOG_DUPLICATE_CONTENT_DIGEST",
			),
		).toBe(true);

		const bakedDuplicatePool = poolFor(
			[sourceFile("01.mp4", "same as baked")],
			{
		...
		expect(bakedDuplicatePool.rejections).toContainEqual(
			expect.objectContaining({
				selectedFile: "01.mp4",
...
describe("REQ-SEL-04 typed oracle-evidence sufficiency", () => {
	...
	const plan = buildCandidateEvidencePlan(
		selectedScenario,
		sourceSha256,
		declaration,
	);
	...
	test("required-missing plus weak supplemental PASS is NA_ASSET independent of detail wording/order", () => {
		...
		expect(first).toMatchObject({
			status: "NA_ASSET",
			sufficient: false,
			reasonCode: "EVIDENCE_NO_SUFFICIENT_SET",
		});
	...
	test("a declared golden-free survivor set carries PASS while unavailable evidence stays visible", () => {
		const survivor = buildCandidateEvidencePlan(
			selectedScenario,
			sourceSha256,
			{
				sourceSha256,
		...
		expect(evaluated).toMatchObject({
			status: "PASS",
			sufficient: true,
			sufficientSurvivorOracles: ["playback-smoke"],
		});
		expect(evaluated.unavailable).toHaveLength(1);
		expect(plan.contractDigest).toMatch(/^[0-9a-f]{64}$/);
		expect(
			plan.requirements.find((item) => item.oracle === "golden-metadata")
				?.needs[0],
		).toEqual({
			kind: "SOURCE_GOLDEN",
			sourceSha256,
...
describe("REQ-SEL-06 fail-closed source/base-bound CENC eligibility", () => {
	...
	test("complete candidate runs only its exact metamorphic invariant", () => {
		...
		const selections = candidatesForRun([cencScenario], sources, {
			bakedManifest: bakedManifest([
				{ id: "cenc-baked.mp4", contents: "baked encrypted" },
			]),
		}).get(cencScenario.id)!;
		...
		expect(
			(real?.effectiveScenario.options as Record<string, unknown>)
				.cleartextAsset,
		).toBeUndefined();
	});
	...
	test("key, base, or evidence mutations are rejected before selection", () => {
		const mutations: SourceFileRecord[] = [
			{ ...encrypted, keys: undefined },
			{
				...encrypted,
				cleartextBase: {
					...encrypted.cleartextBase!,
					sha256: sha256Hex("different base"),
				},
			},
		...
		for (const file of mutations) {
			...
			if (!eligibility.eligible)
				expect(eligibility.rejection.reasonCode).toMatch(/^CENC_/);
		}
	});
});
...
describe("REQ-SEL-06 protected probe DERIVED eligibility", () => {
	...
	test("a source-bound protected probe keeps probe semantics and its metadata oracle", () => {
		...
		const selections = candidatesForRun([protectedProbe], sources, {
			bakedManifest: bakedManifest([
				{ id: "cenc-baked.mp4", contents: "baked protected bytes" },
			]),
		...
		expect(
			(derived?.effectiveScenario.options as Record<string, unknown>).invariant,
		).toBeUndefined();
	});
...
describe("REQ-SEL-07 full-digest pool/execution/cache contracts", () => {
	test("prefix collisions and same filename/different bytes cannot share a cache tag", () => {
		...
		expect(selectionCacheTag(selection(a))).not.toBe(
			selectionCacheTag(selection(b)),
		);
	...
	test("canonical exhaustive set cache survives reorder but changes on member drift", () => {
		const selections = candidatesForRun(
			...
			new Map([
				[
					"probe/selection",
					row([sourceFile("01.mp4", "one"), sourceFile("02.mp4", "two")]),
				],
			]),
			{ bakedManifest: bakedManifest() },
		).get("probe/selection")!;
		expect(exhaustiveSelectionCacheTag(selections)).toBe(
			exhaustiveSelectionCacheTag([...selections].reverse()),
		);
		expect(exhaustiveSelectionCacheTag(selections)).not.toBe(
			exhaustiveSelectionCacheTag(selections.slice(1)),
		);
	});
	...
	test("scenario/oracle contracts invalidate observation keys; current run re-envelopes one observation", () => {
		...
		expect(
			computeObservationKey({
				...base,
				scenarioContractDigest: sha256Hex("scenario-v2"),
			}),
		).not.toBe(key);
		expect(
			computeObservationKey({
				...base,
				oracleEvidenceContractDigest: sha256Hex("oracle-v2"),
			}),
		).not.toBe(key);
	...
	test("baked byte changes alter pool, executed, corpus, and cache identities", () => {
		const emptyCatalog = catalogFromRows([
			row([] as SourceFileRecord[], { class: "SYNTHETIC" }),
		]);
		...
		expect(poolA.candidates[0]?.contentDigest).not.toBe(
			poolB.candidates[0]?.contentDigest,
		);
		const sources = new Map([
			["probe/selection", row([], { class: "SYNTHETIC" })],
		]);
		const selA = selectForRun([scenario()], sources, {
			bakedManifest: bakedManifest([{ id: "baked.mp4", contents: "baked-v1" }]),
		}).get("probe/selection")!;
		const selB = selectForRun([scenario()], sources, {
			bakedManifest: bakedManifest([{ id: "baked.mp4", contents: "baked-v2" }]),
		}).get("probe/selection")!;
		...
		expect(computeCorpusChecksum([selA])).not.toBe(
			computeCorpusChecksum([selB]),
		);
		expect(computeEligiblePoolsDigest([selA])).not.toBe(
			computeEligiblePoolsDigest([selB]),
		);
	});
});
...
describe("REQ-SEL-08 property/e2e selection surface and FEAT-76 robustness vector", () => {
	test("candidatesForRun and report JSON retain canonical pool/evidence/rejection identities", () => {
		...
		const selections = candidatesForRun(
			[scenario()],
			new Map([[row(files).scenarioId, row(files)]]),
			{
				bakedManifest: bakedManifest(),
			},
		).get("probe/selection")!;
		...
		expect(
			selections.every(
				(selection) => selection.evidencePlan && selection.eligiblePoolDigest,
			),
		).toBe(true);
		...
		expect(serialized.selections[0]?.evidencePlan?.requiredOracles).toEqual([
			"golden-metadata",
		]);
	});
	...
	test("three eligible robustness variants require exact same-contract, source-bound sufficient evidence", () => {
		...
		const scenarioContractDigest = sha256Hex(
			"robustness scenario definition v2",
		);
		...
		const files = [
			curated("01.mp4", "corrupt-header"),
			curated("02.mp4", "truncated-payload"),
		];
		const robustnessRow = row(files, {
			scenarioId: robustnessScenario.id,
			class: "REAL",
		});
		const manifest = manifestFor(files, {
			...
			row: robustnessRow,
			baked: bakedManifest([
				{ id: "robustness-baked.mp4", contents: "baked malformed" },
			]),
			scenarioContractDigest,
		});
		...
		expect(
			pool.candidates
				.map((candidate) => candidate.contentDigest)
				.filter((digest, index, all) => all.indexOf(digest) === index),
		).toHaveLength(3);
		expect(
			pool.candidates.every(
				(candidate) => candidate.evidencePlan.sufficientOracleSets.length > 0,
			),
		).toBe(true);

		const arbitrary = sourceFile("03.mp4", "arbitrary corruption");
		const rejected = assessRobustnessVariantEligibility(
			robustnessScenario,
			arbitrary,
			scenarioContractDigest,
		);
		expect(rejected).toMatchObject({
			eligible: false,
			rejection: { reasonCode: "ROBUSTNESS_VARIANT_CONTRACT_MISSING" },
		});
	});
});
