...
import {
	SCENARIO_DEFINITION_V2_SCHEMA,
	RESULTS_V2_JSON_SCHEMA,
} from "../src/core/dsl-schemas.ts";
...
function cloneDefinition(
	scenario = defineScenario(baseSpec()),
): ScenarioDefinitionV2 {
...
const verdicts = {
	pass: {
		...
		verdict: "PASS",
		reasonCode: "METADATA_MATCH",
	},
	diff: {
		state: "VERDICT",
		oracle: "golden-metadata",
		verdict: "PASS",
		reasonCode: "VALID_CODEC_ALIAS",
	},
	fail: {
		state: "VERDICT",
		oracle: "golden-metadata",
	...
	browser: {
		...
		detail: "exact decoder config is unsupported",
	},
	asset: {
		state: "UNAVAILABLE",
		oracle: "golden-metadata",
...
describe("REQ-DSL-01: validated immutable ScenarioDefinitionV2", () => {
	test("the complete canonical battery validates with real asset resolution and zero diagnostics", async () => {
		...
		const diagnostics = validateScenarioBattery(allScenarios, {
			assetExists: (assetId, scenarioId) =>
				manifestAssets.has(assetId) ||
	...
	test("every formerly omitted invariant names the scenario and exact field before registration", () => {
		const fixtures: Array<{
			...
			{
				name: "illegal option",
				path: "options.illegal",
				mutate: (d) => {
					d.options.illegal = true;
				},
			},
			{
				name: "unusable primary metric",
				path: "primaryMetric",
				mutate: (d) => {
			...
			},
			{
				...
				path: "tolerances.ssimMin",
				mutate: (d) => {
					d.tolerances.ssimMin = Number.NaN;
				},
			},
			{
		...
		for (const fixture of fixtures) {
			...
			const diagnostics = validateScenarioDefinitionV2(
				invalid,
				fixture.context,
			);
			expect(
			...
			expect(
				() => registerScenarios([invalid as unknown as Scenario]),
				fixture.name,
			).toThrow();
			expect(
	...
	test("definition input is cloned and the committed snapshot is deeply frozen", () => {
		...
		const spec: ScenarioSpec = {
			...baseSpec("probe/immutable-snapshot"),
			inputs,
			options,
			requires: { operations },
		};
		...
		expect(
			Object.isFrozen((stored.options as { invariant: object }).invariant),
		).toBe(true);
	});
	...
	test("candidate envelopes are canonical input-selection contracts with ordered finite bounds", () => {
		...
		authored.excludeDimensions[0]!.width = 999;
		...
		expect(scenarioDefinitionProjection(bounded).candidateEnvelope).toEqual(
			bounded.candidateEnvelope,
		);
		expect(Object.isFrozen(bounded.candidateEnvelope)).toBe(true);

		const invalid = cloneDefinition();
		invalid.candidateEnvelope = {
			minWidth: 1919.5,
			minDurationSec: 133,
			maxDurationSec: 132,
			minAudioMeanVolumeDb: Number.NEGATIVE_INFINITY,
			excludeDimensions: [
				{ width: 0, height: 240 },
		...
		expect(validateScenarioDefinitionV2(invalid)).toEqual(
			expect.arrayContaining([
				...
				expect.objectContaining({
					path: "candidateEnvelope.minAudioMeanVolumeDb",
					code: "CANDIDATE_ENVELOPE_AUDIO_LEVEL",
				}),
				expect.objectContaining({
					path: "candidateEnvelope.excludeDimensions[0].width",
					code: "CANDIDATE_ENVELOPE_EXCLUDED_DIMENSION",
				}),
				expect.objectContaining({
					path: "candidateEnvelope.excludeDimensions[2]",
					code: "CANDIDATE_ENVELOPE_EXCLUDED_DIMENSION_DUPLICATE",
				}),
			]),
		);
	});
	...
	test("quality-constrained ABR rungs validate as an atomic hard-rate and SSIM contract", () => {
		...
		const rung = () =>
			(
				structuredClone(valid.options.variants) as Array<
		...
		const invalidCases: Array<{
		...
		}> = [
			{
				...
				mutate: (definition) => {
					delete (
						definition.options.variants as Array<Record<string, unknown>>
					)[0]!.quality;
				},
			},
			{
				...
				mutate: (definition) => {
					(
						(definition.options.variants as Array<Record<string, unknown>>)[0]!
			...
			},
			{
				...
				mutate: (definition) => {
					(definition.options.video as Record<string, unknown>).codec = "vp9";
				},
			},
		];
		...
		for (const fixture of invalidCases) {
			...
			fixture.mutate(invalid);
			expect(validateScenarioDefinitionV2(invalid), fixture.code).toEqual(
				expect.arrayContaining([
	...
	test("declarative mutation handlers resolve only after validation", () => {
		...
		const scenario = defineScenario({
			id: "robustness/dsl-mutation",
			family: "robustness",
			op: "probe",
			input: "truncated_ftyp.bin",
			options: {},
		...
		expect(scenario.mutate?.(new Uint8Array([1, 2, 3, 4]))).toEqual(
			new Uint8Array([1, 2]),
		);
		expect(scenarioDefinitionProjection(scenario).mutation).toEqual({
			mutationId: MUTATION_HANDLER,
			parameters: { keepBytes: 2 },
		});
	});
});
...
describe("REQ-DSL-02: tuple alternatives and concrete WebCodecs recipes", () => {
	...
	test("a complete concrete tuple selects one conjunctive alternative and rejects a cross-product miss", () => {
		...
		expect(matchRequirementCombination(scenario.requires, request)).toEqual(
			combination,
		);
		expect(
			matchRequirementCombination(scenario.requires, {
				...request,
				output: { ...request.output!, videoCodec: "av1" },
			}),
		).toBeUndefined();
		expect(
			matchRequirementCombination(scenario.requires, {
				...request,
				options: { ...request.options, container: "mp4" },
			}),
		).toBeUndefined();
	});
	...
	test("parser-only alternatives derive no browser gate", () => {
		const parserOnly: RequirementCombination = {
			operation: "probe",
			containersIn: ["mp4"],
		};
		expect(
			deriveWebCodecsConfigs(parserOnly, { ...request, operation: "probe" }),
		).toEqual([]);
	});
});
...
describe("REQ-DSL-03/06: result schema v2 and explicit v1 migration", () => {
	test("PASS results including recorded representation differences are benchmark eligible while FAIL is correctness-gated", () => {
		expect(
			validateScenarioResultV2(
				resultV2("PASS", [verdicts.pass], { bench: bench() }),
			),
		).toEqual([]);
		expect(
			validateScenarioResultV2(
				resultV2("PASS", [verdicts.diff], { bench: bench() }),
			),
		).toEqual([]);
		expect(
			validateScenarioResultV2(
				resultV2("FAIL", [verdicts.fail], { bench: bench() }),
			).some((entry) => entry.code === "BENCH_CORRECTNESS_GATE"),
		).toBe(true);
	});
		...
		const source = envelope([
			resultV2("PASS", [verdicts.diff], {
				...
				env: {
					...
					pixelBehavior: {
						state: "SUPPORTED",
						reasonCode: "PIXELS_OK",
						detail: "RGBA round trip",
					},
				},
	...
	test("malformed and unknown-major inputs fail before consumption", () => {
		expect(() =>
			readResultsEnvelope({
				schema: "media-browser-test/results@9",
				results: [],
			}),
		).toThrow(ResultSchemaError);
		const malformed = envelope([resultV2("PASS", [verdicts.fail])]);
		expect(() => readResultsEnvelope(malformed)).toThrow(
			/reduce to FAIL, not PASS/,
		);
	});
	...
	test("v1 booleans migrate conservatively and never manufacture DIFF", () => {
					...
					oracleOutcomes: [
						{
							oracle: "golden-metadata",
							pass: false,
							detail: "golden absent for fixture",
						},
					],
				},
			],
		});
		expect(migrated.results.map((result) => result.status)).toEqual([
			"PASS",
			"FAIL",
			"NA_ASSET",
		]);
		expect(
			migrated.results
				.flatMap((result) => result.oracleOutcomes)
				.some(
					(outcome) =>
						outcome.state === "VERDICT" && outcome.verdict === "DIFF",
				),
		).toBe(false);
		expect(
			migrated.results.map((result) => result.instance.inputSha256),
		).toEqual([null, null, null]);
		expect(migrated.migration).toEqual({
			from: "media-browser-test/results@1",
			missingInputDigests: 3,
		});
	});
	...
	test("v1 thin exhaustive rows retain each typed status with transparent synthetic evidence", () => {
			...
			results: [
				{
					engineId: "legacy@1",
					browser: "chromium",
					scenarioId: "probe/legacy-exhaustive",
					family: "probe",
					status: "FAIL",
					oracleOutcomes: [
						{
		...
		expect(result.exhaustive?.map((entry) => entry.status)).toEqual([
			"PASS",
			"FAIL",
			"NA_BROWSER",
		]);
		expect(
			result.exhaustive?.map((entry) => entry.oracleOutcomes[0]?.state),
		).toEqual(["VERDICT", "VERDICT", "UNAVAILABLE"]);
		expect(result.coverage).toEqual(
			reduceExhaustiveStatuses(["PASS", "FAIL", "NA_BROWSER"]).coverage,
		);
	});
	...
	test("exhaustive rows and coverage cannot be detached at a v2 read boundary", () => {
		...
		expect(
			validateScenarioResultV2(
				resultV2("PASS", [verdicts.pass], {
					coverage: reduction.coverage,
				}),
			).some((entry) => entry.code === "EXHAUSTIVE_COVERAGE_PAIR"),
		).toBe(true);
	});
	...
	test("operation evidence has a closed disposition and stage vocabulary", () => {
		...
		expect(
			validateScenarioOperationEvidence({
				...valid,
				disposition: "sort-of-rejected",
			}).some((entry) => entry.code === "OPERATION_DISPOSITION"),
		).toBe(true);
		expect(
			validateScenarioOperationEvidence({ ...valid, stage: "somewhere" }).some(
				(entry) => entry.code === "OPERATION_STAGE",
			),
		).toBe(true);
		expect(() =>
			readResultsEnvelope(
				envelope([
					resultV2("PASS", [verdicts.pass], {
						operationEvidence: { ...valid, stage: "somewhere" as never },
					}),
				]),
			),
		).toThrow(ResultSchemaError);
	...
	test("typed candidate sufficiency and full selection identities survive the strict v2 boundary", () => {
		const source = resultV2("NA_ASSET", [verdicts.pass], {
			...
			selection: {
				...
				isBaked: false,
				candidateCount: 2,
				eligiblePoolDigest: SHA_B,
				executedInputDigest: "c".repeat(64),
				...
				evidenceContractDigest: "f".repeat(64),
				catalogState: "ready",
			},
			candidateEvidence: {
		...
		expect(
			...
			}).some((entry) => entry.code === "SCHEMA_ADDITIONAL_PROPERTY"),
		).toBe(true);
		expect(
			validateScenarioResultV2({
				...source,
				candidateEvidence: { ...source.candidateEvidence!, sufficient: "yes" },
			}).some((entry) => entry.code === "CANDIDATE_EVIDENCE_SUFFICIENT"),
		).toBe(true);
		...
		expect(
			validateScenarioResultV2(independentStreamingFailure).some(
				(entry) => entry.code === "CANDIDATE_EVIDENCE_RESULT_MISMATCH",
			),
		).toBe(false);
		expect(
			readResultsEnvelope(envelope([independentStreamingFailure])).results[0]
				?.status,
		).toBe("FAIL");
		expect(
			validateScenarioResultV2({
				...
				status: "PASS",
			}).some((entry) => entry.code === "CANDIDATE_EVIDENCE_RESULT_MISMATCH"),
		).toBe(true);
	});
});
...
describe("REQ-DSL-05: one canonical manifest and atomic registration", () => {
	test("the complete 592-scenario battery passes the production hash/freeze registration gate", () => {
		...
		for (const scenario of registered) {
			expect(
				hashScenarioDefinition(scenarioDefinitionProjection(scenario)),
				scenario.id,
	...
	test("eager and lazy consumers produce byte-identical ordered identities", async () => {
		...
		const reversedSourceOrder = await loadCanonicalScenarios(
			[...SCENARIO_FAMILY_MANIFEST].reverse(),
		);
		expect(
			reversedSourceOrder.map(({ id, definitionHash }) => ({
				id,
				definitionHash,
			})),
		).toEqual(lazy.map(({ id, definitionHash }) => ({ id, definitionHash })));
		const robustness = lazy.findIndex(
			(scenario) => scenario.family === "robustness",
		);
		const performance = lazy.findIndex(
			(scenario) => scenario.family === "performance",
		);
	...
	test("a duplicate in one batch is atomic and reports the exact family/member", () => {
		...
		expect(() => registerScenarios([first, first])).toThrow(
			/family 'probe'.*member 'probe\/duplicate-member'/,
		);
...
describe("REQ-DSL-07: explicit input identity and partial coverage", () => {
	...
	test("mixed input statuses reduce deterministically with first-class coverage grades", () => {
		const cases: Array<{
			...
			{
				statuses: ["PASS", "NA_ENGINE"],
				status: "PASS",
				grade: "full",
				valid: 1,
			},
			{
				statuses: ["NA_ENGINE", "NA_ENGINE"],
				status: "NA_ENGINE",
				grade: "none",
				valid: 0,
			},
			{
				statuses: ["NA_BROWSER", "PASS"],
				status: "PASS",
				grade: "partial",
				valid: 1,
			},
			{
				statuses: ["NA_BROWSER", "NA_ASSET"],
				status: "NA_BROWSER",
				grade: "none",
				valid: 0,
			},
	...
	test("intrinsically unsupported variants preserve full executable coverage at the strict v2 boundary", () => {
		...
		const source = resultV2("PASS", [verdicts.pass], {
			reason:
				"coverage full 1/1 executable variants; 01.mp4(PASS); attachment.mov(NA_ENGINE)",
			exhaustive: [
				{
					file: "01.mp4",
					sha256: SHA_A,
				...
				},
				{
					...
					status: "NA_ENGINE",
					reason:
						"[MEDIABUNNY_TRACK_TYPE_UNSUPPORTED] attachment tracks are outside the demux API",
					oracleOutcomes: [],
					measurement: { state: "NOT_REQUESTED" },
					operationEvidence,
					executed: true,
...
describe("REQ-DSL-08: explicit expansion snapshot", () => {
	test("the committed expansion identity matches and is independent of source array order", async () => {
		const expected = await Bun.file(
			join(ROOT, "schemas/scenario-expansion.snapshot.json"),
		).json();
		expect(compareScenarioExpansionSnapshot(expected, allScenarios)).toEqual({
			matches: true,
		});
		expect(
			serializeScenarioExpansionSnapshot(
				buildScenarioExpansionSnapshot([...allScenarios].reverse()),
			),
		).toBe(
			serializeScenarioExpansionSnapshot(
				buildScenarioExpansionSnapshot(allScenarios),
			),
		);
	});
	...
	test("adding or removing a builder row changes the checked snapshot", async () => {
		...
		expect(
			compareScenarioExpansionSnapshot(expected, allScenarios.slice(1)).matches,
		).toBe(false);
		const added = defineScenario(baseSpec("probe/expansion-added-row"));
		expect(
			compareScenarioExpansionSnapshot(expected, [...allScenarios, added])
				.matches,
		).toBe(false);
	});

	test("published Draft 2020-12 schema files exactly match their source constants", async () => {
		const scenarioSchema = await Bun.file(
			join(ROOT, "schemas/scenario-definition-v2.schema.json"),
		).json();
		const resultsSchema = await Bun.file(
			join(ROOT, "schemas/results-v2.schema.json"),
		).json();
		expect(scenarioSchema).toEqual(SCENARIO_DEFINITION_V2_SCHEMA);
		expect(resultsSchema).toEqual(RESULTS_V2_JSON_SCHEMA);
		expect(scenarioSchema.$schema).toBe(
			"https://json-schema.org/draft/2020-12/schema",
		);
		expect(resultsSchema.$schema).toBe(
			"https://json-schema.org/draft/2020-12/schema",
		);
	});
});
